...
class Xcrud
{
  ...
  protected $tabulator_column_properties = [];
  ...
  protected $before_remove = [];
  protected $after_insert = [];
  protected $after_update = [];
  protected $after_remove = [];
  protected $field_type = [];
  protected $field_attr = [];
  protected $defaults = [];
  protected $limit = 20;
  protected $limit_list = ["20", "50", "100", "all"];
  protected $column_cut = 50;
  protected $column_cut_list = [];
  protected $no_editor = [];
  ...
  protected $upload_to_save = [];
  protected $upload_to_remove = [];
  protected $binary = [];
  protected $pass_var = [];
  protected $reverse_fields = [];
  protected $no_quotes = [];
  protected $join = [];
  protected $inner_where = [];
  protected $inner_table_instance = [];
  protected $inner_table_template = "";
  protected $condition = [];
  protected $theme = "default";
  protected $unique = [];
  protected $fk_relation = [];

  protected $links_ = [];
  protected $emails_label = [];
  protected $sum = [];
  protected $alert_create;
  protected $alert_edit;
  protected $subselect = [];
  protected $subselect_before = [];

  protected $highlight = [];
  protected $highlight_row = [];
  protected $modal = [];
  protected $column_class = [];
  protected $no_select = []; // only subselect flag for correct sorting
  protected $primary_ai = false;

  protected $language = "en";

  protected static $lang_arr = [];

  protected $subselect_query = [];
  protected $where_pri = [];
  protected $field_params = [];
  protected $mass_alert_create = [];
  protected $mass_alert_edit = [];
  protected $send_email_public = [];
  protected $column_callback = [];
  protected $field_callback = [];
  protected $replace_insert = [];
  protected $replace_update = [];
  protected $replace_remove = [];
  protected $send_external_create = [];
  protected $send_external_edit = [];
  protected $locked_fields = []; // disallow save data in form fields
  protected $column_pattern = [];
  protected $field_tabs = [];
  protected $fields_arrangement = [];
  protected $field_marker = [];
  protected $field_tooltip = [];
  protected $table_tooltip = [];
  protected $column_tooltip = [];
  protected $search_columns = [];
  protected $search_default = null;
  protected $column_width = [];
  protected $column_hide = [];
  ...
  protected $result_list = []; // DB grid result becomes glodal
  protected $result_row = []; // DB details result becomes glodal
  ...
  protected $before_upload = [];
  protected $after_upload = [];
  protected $after_resize = [];
  ...
  protected $hidden_columns = []; // allows to select non in grid data
  protected $hidden_fields = []; // allows save data in non in form fields
  protected $range = "";
  protected $advanced = "";
  ...
  protected $hidden_fields_output = [];
  protected $start = 0;
  protected $before = "";
  protected $bit_field = [];
  protected $point_field = [];
  protected $float_field = [];
  protected $text_field = [];
  protected $int_field = [];
  protected $grid_condition = []; // ***** remove *****
  protected $hide_button = [];
  protected $set_lang = [];
  ...
  protected $grid_restrictions = [];
  protected $direct_select_tags = []; // get unselectable {tags}
  protected $action = [];

  protected $exception = false;
  protected $exception_fields = [];
  protected $exception_text = "";
  protected $links_label = "";
  protected $message = [];
  protected $callback_url = "";

  protected $nested_rendered = [];
  protected $default_tab = false;
  protected $prefix = "";
  protected $query = "";
  protected $total_query = "";

  protected $condition_backup = [];
  ...
  protected $before_list = [];
  protected $before_create = [];
  protected $before_edit = [];
  protected $before_view = [];
  ...
  {
    Xcrud_config::$scripts_url = self::check_url(
      Xcrud_config::$scripts_url,
      true
    );
    Xcrud_config::$editor_url = self::check_url(Xcrud_config::$editor_url);
    Xcrud_config::$editor_init_url = self::check_url(
      Xcrud_config::$editor_init_url
    );
    ...
    $this->date_format = [
      "php_d" => Xcrud_config::$php_date_format,
      "php_t" => Xcrud_config::$php_time_format,
    ];
  }
  ...
  {
    if (
      isset($_POST["xcrud"]["instance"]) &&
      isset($_POST["xcrud"]["key"]) &&
      isset($_POST["xcrud"]["task"])
    ) {
      self::init_prepare("post");
      $key = $_POST["xcrud"]["key"]
        ? $_POST["xcrud"]["key"]
        : self::error("Security key cannot be empty");
      $inst_name = $_POST["xcrud"]["instance"]
        ? $_POST["xcrud"]["instance"]
        : self::error("Instance name cannot be empty");
      $is_get = false;
    } elseif (
      isset($_GET["xcrud"]["instance"]) &&
      isset($_GET["xcrud"]["key"]) &&
      isset($_GET["xcrud"]["task"]) &&
      $_GET["xcrud"]["task"] == "file"
    ) {
    ...
    if (
      isset($_SESSION["lists"]["xcrud_session"][$inst_name]["key"]) &&
      $_SESSION["lists"]["xcrud_session"][$inst_name]["key"] == $key
    ) {
  ...
  protected static function init_prepare($method = false)
  {
    switch ($method) {
      case "post":
        $sess_name =
          Xcrud_config::$dynamic_session &&
          isset($_POST["xcrud"]["sess_name"]) &&
          $_POST["xcrud"]["sess_name"]
            ? $_POST["xcrud"]["sess_name"]
      ...
      case "get":
        $sess_name =
          Xcrud_config::$dynamic_session &&
          isset($_GET["xcrud"]["sess_name"]) &&
          $_GET["xcrud"]["sess_name"]
            ? $_GET["xcrud"]["sess_name"]
            : Xcrud_config::$sess_name;
        break;
      default:
  ...
  public static function session_start($sess_name = false)
  {
    ...
    if (
      !session_id() &&
      !Xcrud_config::$external_session &&
      !Xcrud_config::$alt_session
    ) {
      if (!headers_sent()) {
    ...
    if (Xcrud_config::$alt_session) {
      if (!headers_sent()) {
        if (!isset($_COOKIE[$sess_name])) {
          self::$sess_id = base_convert(
            str_replace(" ", "", microtime()) . rand(),
            10,
            36
          );
        } else {
        ...
        setcookie(
          $sess_name,
          self::$sess_id,
          time() + Xcrud_config::$alt_lifetime * 60,
          "/"
        );
      } else {
  ...
  public function connection(
    ...
    if ($user && $table) {
      $this->connection = [$user, $pass, $table, $host, $encode];
    }
    return $this;
  }
  ...
  public function translate_external_text($text = "")
  {
    ...
    if (
      is_file(
        XCRUD_PATH .
          "/" .
          Xcrud_config::$lang_path .
          "/" .
          $this->language .
          ".ini"
      )
    ) {
      $tmp_lang_arr = parse_ini_file(
        XCRUD_PATH .
          "/" .
          Xcrud_config::$lang_path .
          "/" .
      ...
      is_file(XCRUD_PATH . "/" . Xcrud_config::$lang_path . "/en.ini")
    ) {
      $tmp_lang_arr = parse_ini_file(
        XCRUD_PATH . "/" . Xcrud_config::$lang_path . "/en.ini"
      );
    }
    if ($this->set_lang) {
    ...
    return htmlspecialchars(
      isset($tmp_lang_arr[$langtext]) ? $tmp_lang_arr[$langtext] : $text,
      ENT_QUOTES,
  ...
  public function limit_list($limit_list = "")
  {
    if ($limit_list) {
      if (is_array($limit_list)) {
        ...
        $this->limit_list = array_unique(
          $this->parse_comma_separated($limit_list)
        );
      }
    }
    return $this;
  ...
  public function table_name($name = "", $tooltip = false, $icon = false)
  {
    ...
    if ($tooltip) {
      $this->table_tooltip = ["tooltip" => $tooltip, "icon" => $icon];
    }
    return $this;
  }
  ...
  public function where(
    ...
    if ($fields && $where_val !== false) {
      ...
      if ($index) {
        $this->where[$index] = ["custom" => $fields, "glue" => $glue];
      } else {
        $this->where[] = ["custom" => $fields, "glue" => $glue];
      }
      unset($fields);
    } elseif (!$fields && $where_val) {
      if ($index) {
        $this->where[$index] = ["custom" => $where_val, "glue" => $glue];
      } else {
        $this->where[] = ["custom" => $where_val, "glue" => $glue];
      }
      unset($where_val);
    } elseif (
      !$fields &&
    ...
    if ($fields) {
      if ($direction === false && is_string($fields)) {
        ...
        foreach ($fdata as $key => $fitem) {
          $this->order_by[$key] = isset($fitem["value"])
            ? $fitem["value"]
            : $direction;
        }
      }
  ...
  public function relation(
    ...
    if ($fields && $rel_tbl && $rel_field && $rel_name) {
      if ($depend_on) {
        ...
        $depend_on = key(
          $fdata
        ) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
      }
      $fdata = $this->_parse_field_names($fields, "relation");
      foreach ($fdata as $fitem) {
        $this->relation[$fitem["table"] . "." . $fitem["field"]] = [
          "rel_tbl" => $rel_tbl,
  ...
  public function fk_relation(
    ...
    if ($fields && $rel_tbl && $rel_field && $rel_name && $label) {
      ...
      $alias = $table . "." . $label;
      $this->fk_relation[$alias] = [
        "table" => $fitem["table"],
        "field" => $fitem["field"],
        "label" => $label,
        "before" => $before
          ? key(reset($this->_parse_field_names($before, "fk_relation")))
          : "",
        "alias" => $alias,
  ...
  public function nested_table(
    ...
    if ($instance_name && $field && $inner_tbl && $tbl_field) {
      ...
      foreach ($fdata as $fitem) {
        $this->inner_table_instance[$instance_name] =
          $fitem["table"] . "." . $fitem["field"]; // name of nested object will be stored in parent instance
        ...
        $fdata2 = $this->_parse_field_names(
          $tbl_field,
          "nested_table",
          $inner_tbl
        );

        $instance->inner_where[$fitem["table"] . "." . $fitem["field"]] = key(
          $fdata2
  ...
  public function fields_arrange(
    $fields = "",
    $groupname = false,
    $label_shown = false
  ) {
  ...
  public function label($fields = "", $label = "")
  {
    ...
    foreach ($fdata as $fitem) {
      $this->labels[$fitem["table"] . "." . $fitem["field"]] = isset(
        $fitem["value"]
      )
        ? $fitem["value"]
        : $label;
  ...
  public function set_bulk_select(
    $bool = true,
    $field = false,
    $operand = false,
    $value = false
    ...
    if ($field && $operand && $value !== false) {
      $this->grid_restrictions["bulk_select"] = [
        "field" => $field,
        "operator" => $operand,
        "value" => $value,
      ];
  ...
  public function duplicate_button(
    ...
    if ($field && $operand && $value !== false) {
      $this->grid_restrictions["duplicate"] = [
        "field" => $field,
        "operator" => $operand,
        "value" => $value,
      ];
      ...
      foreach ($fdata as $fitem) {
        switch ($type) {
          ...
          case "price":
            ...
            $this->field_attr[
              $fitem["table"] . "." . $fitem["field"]
            ] = array_merge($def_attr, (array) $attr);
            break;
          case "button":
          case "select":
          case "multiselect":
          ...
          case "checkboxes":
            if (!is_array($attr) or !isset($attr["values"])) {
              $this->field_attr[$fitem["table"] . "." . $fitem["field"]][
                "values"
              ] = $attr;
            } else {
              $this->field_attr[
  ...
  public function pass_default($fields = "", $value = "")
  {
    ...
    foreach ($fdata as $fitem) {
      $this->defaults[$fitem["table"] . "." . $fitem["field"]] = isset(
        $fitem["value"]
      )
        ? $fitem["value"]
        : $value;
  ...
  public function readonly($fields = "", $mode = false)
  {
  ...
  public function column_cut($int = 50, $fields = false, $safe_output = false)
  {
    if ($fields === false) {
      ...
      foreach ($fdata as $fitem) {
        $this->column_cut_list[$fitem["table"] . "." . $fitem["field"]] = [
          "count" => $int,
          "safe" => $safe_output,
        ];
      }
  ...
  public function validation_required($fields = "", $chars = 1)
  {
    ...
    foreach ($fdata as $fitem) {
      $this->validation_required[
        $fitem["table"] . "." . $fitem["field"]
      ] = isset($fitem["value"]) ? $fitem["value"] : $chars;
    }
    return $this;
  }
  public function validation_pattern($fields = "", $pattern = "")
  {
    ...
    foreach ($fdata as $fitem) {
      $this->validation_pattern[
        $fitem["table"] . "." . $fitem["field"]
      ] = isset($fitem["value"]) ? $fitem["value"] : $pattern;
    }
    return $this;
  }
  ...
  public function page_call_create(
    $url = "",
    $data = [],
    $where_param = "",
    $where_value = "",
  ...
  public function page_call_edit(
    $url = "",
    $data = [],
    $where_param = "",
    $where_value = "",
  ...
  public function highlight(
    ...
    if ($columns && $operator) {
      ...
      foreach ($fdata as $fitem) {
        $this->highlight[$fitem["table"] . "." . $fitem["field"]][] = [
          "value" => $value,
          "operator" => $operator,
          "color" => $color,
          "class" => $class,
  ...
  public function modal($columns = "", $icon = false)
  {
    ...
    foreach ($fdata as $fitem) {
      $this->modal[$fitem["table"] . "." . $fitem["field"]] = isset(
        $fitem["value"]
      )
        ? $fitem["value"]
        : $icon;
  ...
  public function column_class($columns = "", $class = "")
  {
    ...
    foreach ($fdata as $fitem) {
      $this->column_class[$fitem["table"] . "." . $fitem["field"]][] = isset(
        $fitem["value"]
      )
        ? $fitem["value"]
        : $class;
    ...
    if ($fields && $type) {
      ...
      foreach ($fdata as $fitem) {
        $this->advanced_filter[
          $fitem["table"] . "." . $fitem["field"] . "." . $order
        ] = [
          "type" => isset($fitem["type"]) ? $fitem["type"] : $type,
          "values" => $values,
        ];
      }
    }
  ...
  public function field_tooltip($fields = "", $tooltip = "", $icon = false)
  {
    if ($fields && $tooltip) {
      ...
      foreach ($fdata as $fitem) {
        $this->field_tooltip[$fitem["table"] . "." . $fitem["field"]] = [
  ...
  public function column_callback(
    ...
    if ($fields && $callback && $path) {
      ...
      foreach ($fdata as $fitem) {
        $this->column_callback[$fitem["table"] . "." . $fitem["field"]] = [
  ...
  public function field_callback(
    ...
    if ($fields && $callback && $path) {
      ...
      foreach ($fdata as $fitem) {
        $this->field_callback[$fitem["table"] . "." . $fitem["field"]] = [
          "name" => $fitem["table"] . "." . $fitem["field"],
          "path" => rtrim($path, "/"),
          "callback" => $callback,
        ];
  ...
  {
    if ($callable) {
      $this->replace_insert = ["callable" => $callable, "path" => $path];
    }
    return $this;
  }
  ...
  {
    if ($callable) {
      $this->replace_update = ["callable" => $callable, "path" => $path];
    }
    return $this;
  }
  ...
  {
    if ($callable) {
      $this->replace_remove = ["callable" => $callable, "path" => $path];
    }
    return $this;
  }
  ...
  public function call_update($postdata, $primary)
  {
    if (!$this->task) {
      self::error(
        "Sorry, but you must use <strong>call_update()</strong> only in callbacks"
      );
    }
  ...
  {
    $this->search_pattern = [$left, $right];
    return $this;
  }
  ...
  {
    if ($fields) {
      $fdata = $this->_parse_field_names(
        $fields,
        "tabulator_column_properties"
      );
      foreach ($fdata as $fitem) {
        $this->tabulator_column_properties[
          $fitem["table"] . "." . $fitem["field"]
        ] = $properties;
      }
    }
  ...
  public function create_action(
    ...
    if ($callable && $name) {
      $this->action[$name] = ["callable" => $callable, "path" => $path];
    }
    return $this;
  }
  public function get($name = "")
  {
    if (!$this->task) {
      self::error(
        "Sorry, but you must use <strong>get()</strong> only in callbacks"
      );
    }
    if ($this->_get("key")) {
  ...
  public function set_attr($fields = "", array $attr = [])
  {
    if ($fields && $attr) {
      $fdata = $this->_parse_field_names($fields, "set_attr");
      foreach ($fdata as $fkey => $fitem) {
        if (isset($this->field_attr[$fkey])) {
          $this->field_attr[$fkey] = array_merge(
            (array) $this->field_attr[$fkey],
            $attr
          );
        } else {
  ...
  protected function _run_task()
  {
    ...
    switch ($this->task) {
      ...
      case "print":
        ...
        break;
      case "depend":
        return $this->create_relation(
          $this->_post("name", false, "base64"),
          $this->_post("value"),
          $this->get_field_attr($this->_post("name", false, "base64"), "edit"),
          $this->_post("dependval")
        );
        break;
      case "view":
  ...
  protected function render_custom_datagrid()
  {
    ...
    $db->query(
      "SELECT COUNT(*) as `count` FROM (SELECT NULL" .
        $this->total_query .
        ") counts"
    );
    $this->sum_row = $db->row();
    ...
    foreach ($this->columns as $key => $tmp) {
      $this->columns[$key] = ["table" => "", "field" => $key];
  ...
  protected function render_custom_csv()
  {
    ...
    $table_name = $this->_clean_file_name(
      trim(html_entity_decode($this->table_name, ENT_QUOTES, "utf-8"))
    );
    header(
    ...
    fputcsv(
      $output,
      $this->columns_names,
      Xcrud_config::$csv_delimiter,
      Xcrud_config::$csv_enclosure
    );
  ...
  protected function parse_query_params()
  {
    ...
    $tmp = preg_replace_callback(
      "/\( (?> [^)(]+ | (?R) )+ \)/xui",
      [$this, "query_params_callback"],
      $query
    );
    $from_pos = mb_strpos($tmp, " FROM ");
    $this->total_query = mb_substr($query, $from_pos);
    $query =
      mb_substr($query, 0, $from_pos) .
      ",(0) AS `primary_key`" .
      mb_substr($query, $from_pos);
    return $query;
  }
  ...
  protected function _render_file()
  {
    ...
    if (!$image) {
      ...
      if (
        ...
        $this->upload_config[$field]["blob"] === true
      ) {
        $blob = true;
      } else {
        if ($thumb !== false) {
      ...
      if (trim(strtolower(strrchr($path, ".")), ".") == "pdf") {
        ...
        header(
          "Content-Disposition: inline; filename=\"" .
            (isset($settings["filename"]) ? $settings["filename"] : $image) .
            "\""
        );
      } else {
        header("Content-type: application/octet-stream");
  ...
  public function _csv()
  {
    ...
    foreach ($this->columns as $field => $fitem) {
      if (
        isset($this->field_type[$field]) &&
        ($this->field_type[$field] == "password" or
    ...
    header(
      "Content-Disposition: attachment; filename=\"" .
        $this->_clean_file_name(
          $this->table_name ? $this->table_name : $this->table
        ) .
        ".csv\""
    );
    header("Content-Transfer-Encoding: binary");
    ...
    $db->query(
      "SELECT {$select} FROM `{$this->table}` {$table_join} {$where} {$order_by}"
    );
    while ($row = $db->result->fetch_assoc()) {
      // low level result process, saves memory
      $out = [];
  ...
  protected function _post($field = "", $default = false, $filter = false)
  {
    if (isset($_POST["xcrud"][$field])) {
      ...
      if (is_array($_POST["xcrud"][$field])) {
        array_walk_recursive($_POST["xcrud"][$field], [
          $this,
      ...
      if (
        ($field == "postdata" or $field == "unique") &&
        $_POST["xcrud"][$field]
      ) {
        $data_keys = array_keys($_POST["xcrud"][$field]);
        foreach ($data_keys as $k => $key) {
          $data_keys[$k] = $xss
            ? $xss->xss_clean($this->fieldname_decode($key))
            : $this->fieldname_decode($key);
          if ($xss) {
            $_POST["xcrud"][$field][$key] = $xss->xss_clean(
              $_POST["xcrud"][$field][$key]
            );
          }
        }
        ...
        switch ($filter) {
          case "key":
            return str_replace(
              ...
              "",
              $xss
                ? $xss->xss_clean($_POST["xcrud"][$field])
                : $_POST["xcrud"][$field]
            );
            break;
          case "int":
          ...
          case "base64":
            return $xss
              ? $xss->xss_clean(
                $this->fieldname_decode($_POST["xcrud"][$field])
              )
              : $this->fieldname_decode($_POST["xcrud"][$field]);
            break;
          default:
  ...
  protected function _get($field = "", $default = false, $filter = false)
  {
    if (isset($_GET["xcrud"][$field])) {
      ...
      if (is_array($_GET["xcrud"][$field])) {
        array_walk_recursive($_GET["xcrud"][$field], [
          $this,
          "stripslashes_callback",
        ]);
      } else {
        ...
        switch ($filter) {
          case "key":
            return str_replace(
              ...
              "",
              $xss
                ? $xss->xss_clean($_GET["xcrud"][$field])
                : $_GET["xcrud"][$field]
            );
            break;
          case "int":
  ...
  protected function _create($postdata = [])
  {
    ...
    if ($this->before_create) {
      ...
      if (is_callable($this->before_create["callable"])) {
        ...
        call_user_func_array($this->before_create["callable"], [
          $postdata,
          $this,
        ]);
    ...
    if ($this->condition) {
      foreach ($this->condition as $params) {
        ...
        $params["value"] = $this->replace_text_variables(
          $params["value"],
          $this->result_row
        );
        if (
          array_key_exists($params["field"], $this->result_row) &&
          $this->_compare(
          ...
          } elseif (is_callable([$this, $params["method"]])) {
            $this->condition_backup($params["method"]);
            call_user_func_array([$this, $params["method"]], $params["params"]);
  ...
  protected function _entry($mode = "edit", $postdata = [])
  {
    $db = Xcrud_db::get_instance($this->connection);
    ...
    if (
      (!$this->is_edit($this->result_row) || $this->table_ro) &&
        ($mode == "edit" && $this->editmode != "inline") or
      !$this->is_view($this->result_row) && $mode == "view"
    ) {
      return self::error("Forbidden");
    }
    ...
    if ($this->{$callback_method}) {
      $path = $this->check_file(
        $this->{$callback_method}["path"],
        $callback_method
      );
      include_once $path;
      if (is_callable($this->{$callback_method}["callable"])) {
        $postdata = new Xcrud_postdata($this->result_row, $this);
        call_user_func_array($this->{$callback_method}["callable"], [
          $postdata,
          $this->primary_val,
          $this,
        ]);
    ...
    if ($this->condition) {
      foreach ($this->condition as $params) {
        ...
        if (
          isset($this->result_row[$params["field"]]) &&
          $this->_compare(
            $this->result_row[$params["field"]],
            $params["operator"],
            $params["value"]
    ...
    if ($mode == "edit" && isset($this->pass_var["edit"])) {
      ...
      if ($data) {
        $this->pass_var["edit"][$pkey][
          "tmp_value"
        ] = $this->replace_text_variables($param["value"], $data);
      }
    }
  ...
  protected function prepare_query_field(
    $val,
    $key,
    $action,
    $no_processing = false
    ...
    if ($no_processing) {
      if (
        ...
        isset($this->pass_var[$action][$key])
      ) {
      ...
      if (is_array($val)) {
        ...
        isset($this->float_field[$key]) &&
        $this->field_type[$key] == "price"
      ) {
  ...
  protected function _insert(
    ...
    foreach ($postdata as $key => $val) {
      if (
        isset($fields[$key]) &&
        !isset($this->locked_fields[$key]) &&
        !isset($this->custom_fields[$key])
      ) {
        if (isset($this->field_type[$key])) {
        ...
        $set[$fields[$key]["table"]][
          "`" . $fields[$key]["field"] . "`"
        ] = $this->prepare_query_field($val, $key, "create");
    ...
    ((isset($this->no_quotes[$key]) &&
    isset($this->pass_var['create'][$key])) ? $db->escape($val, true) : $db->escape($val, false,
    $this->field_type[$key], $this->
    field_null[$key], isset($this->bit_field[$key])));*/
        $set[$no_processing_fields[$key]["table"]][
          "`" . $no_processing_fields[$key]["field"] . "`"
        ] = $this->prepare_query_field($val, $key, "create", true);
      }
    }
    ...
    if (
      !$this->primary_ai &&
      !isset($postdata[$this->table . "." . $this->primary_key])
    ) {
    ...
    if (!$this->demo_mode) {
      $db->query(
        "INSERT INTO `" .
          $this->table .
          "` (" .
          implode(",", array_keys($set[$this->table])) .
          ") VALUES (" .
    ...
    if (!$this->demo_mode && $this->is_log == true) {
      $q_log =
        ...
        ")";
      $db->query(
        'INSERT INTO `logs` (record_id,action,table_name,data) VALUES ("' .
          $ins_id .
          '","INSERT","' .
          $this->table .
          '","' .
    ...
    if ($this->join) {
      foreach ($this->join as $alias => $param) {
        $set[$alias]["`" . $param["join_field"] . "`"] =
          $set[$param["table"]]["`" . $param["field"] . "`"];
        if (!$this->demo_mode && !$param["not_insert"]) {
          $db->query(
            "INSERT INTO `{$param["join_table"]}` (" .
              implode(",", array_keys($set[$alias])) .
              ") VALUES (" .
              implode(",", $set[$alias]) .
              ")"
          );
    ...
    if ($this->fk_relation) {
      foreach ($this->fk_relation as $fk) {
        ...
        if (
          array_key_exists($fk["alias"], $postdata) &&
          array_key_exists($field, $postdata)
        ) {
          $in_val = $db->escape(
            $postdata[$field],
            false,
            $this->field_type[$field],
            $this->field_null[$field],
            isset($this->bit_field[$field])
          );
          $db->query(
            "DELETE FROM `" .
              $fk["fk_table"] .
          ...
          if ($fkids) {
            ...
            foreach ($fkids as $fkid) {
              $ins_vals[] =
                "(" . implode(",", $ins_add) . "," . $db->escape($fkid) . ")";
            }
            $db->query(
              "INSERT INTO `" .
                $fk["fk_table"] .
  ...
  protected function _update($postdata, $primary)
  {
    ...
    {
    $set[] = '`' . $fields[$key]['table'] . '`.`' . $fields[$key]['field'] . '` = Point(' . $db->escape($val, true,
    'point',
    $this->field_null[$key], isset($this->bit_field[$key])) . ')';
    }
    else
    $set[] = '`' . $fields[$key]['table'] . '`.`' . $fields[$key]['field'] . '` = ' . ((isset($this->no_quotes[$key]) &&
    isset($this->pass_var['edit'][$key])) ? $db->escape($val, true) : $db->escape(trim($val), false,
    $this->field_type[$key],
    $this->field_null[$key], isset($this->bit_field[$key])));
    */
        $set[] =
          ...
          $fields[$key]["field"] .
          "` = " .
          $this->prepare_query_field($val, $key, "edit");
      }
    }
    if (!$set) {
    ...
    ON `{$param["table"]}`.`{$param["field"]}` = `{$alias}`.`{$param["join_field"]}`";
      ...
      if (!$this->demo_mode) {
        $res = $db->query(
          "UPDATE `{$this->table}` AS `{$this->table}` " .
            implode(" ", $joins) .
            " SET " .
            implode(",\r\n", $set) .
    ...
    if (!$this->demo_mode && $this->is_log == true) {
      $q_log =
        ...
        $db->escape($primary);
      $db->query(
        "INSERT INTO `logs` (record_id,action,table_name,data,old_record) VALUES (" .
          $db->escape($primary) .
          ',"UPDATE","' .
          $this->table .
          '","' .
        ...
        if (
          ...
          $db->query(
            "DELETE FROM `" .
              ...
              $in_val .
              " AND " .
              $this->_build_rel_ins_where($fk["alias"])
          );
          $fkids = $this->parse_comma_separated($postdata[$fk["alias"]]);
          if ($fkids) {
            $ins_vals = [];
  ...
  protected function _remove()
  {
    ...
    if ($this->before_remove) {
      ...
      if (is_callable($this->before_remove["callable"])) {
        call_user_func_array($this->before_remove["callable"], [
          $this->primary_val,
          $this,
        ]);
        if ($this->exception) {
    ...
    if ($this->replace_remove) {
      $path = $this->check_file(
        $this->replace_remove["path"],
        "replace_remove"
      );
      include_once $path;
      if (is_callable($this->replace_remove["callable"])) {
        $this->primary_val = call_user_func_array(
          $this->replace_remove["callable"],
          [$this->primary_val, $this]
        );
      }
    } else {
      // remove case
      ...
      if ($this->direct_select_tags) {
        // tags for unset condition
        foreach ($this->direct_select_tags as $key => $dsf) {
          $fields[$key] = "`{$dsf["table"]}`.`{$dsf["field"]}` AS `{$key}`";
        }
      }
      if (
        in_array("image", $this->field_type) or
        in_array("file", $this->field_type) or
        in_array("fk_relation", $this->field_type)
      ) {
        // images && fk
        foreach ($this->field_type as $key => $type) {
          switch ($type) {
            ...
            case "fk_relation":
              $fields[
                $this->fk_relation[$key]["table"] .
                  "." .
                  $this->fk_relation[$key]["field"]
              ] =
                "`" .
                $this->fk_relation[$key]["table"] .
                "`.`" .
      ...
      $db->query(
        "SELECT * FROM `{$this->table}` WHERE `{$this->primary_key}` = " .
          $db->escape($this->primary_val) .
          "
    LIMIT 1"
      );
      //echo "SELECT * FROM `{$this->table}` WHERE `{$this->primary_key}` = " . $db->escape($this->primary_val) . " LIMIT
      //1";
      $row_log = $db->row();
      $row_log_json = implode(" | ", $row_log);
    ...
    ON `{$param["table"]}`.`{$param["field"]}` = `{$alias}`.`{$param["join_field"]}`";
      ...
      if ($del_row && !$this->demo_mode) {
        foreach ($del_row as $key => $val) {
          if (
            $val &&
            isset($this->upload_config[$key]) &&
            !isset($this->upload_config[$key]["blob"])
          ) {
    ...
    if ($this->after_remove) {
      ...
      if (is_callable($this->after_remove["callable"])) {
        call_user_func_array($this->after_remove["callable"], [
          $this->primary_val,
          $this,
        ]);
      }
  ...
  protected function check_postdata($postdata, $primary)
  {
    ...
    foreach ($postdata as $key => $val) {
      if (
        isset($this->disabled[$key][$mode]) &&
        !isset($this->readonly[$key][$mode])
      ) {
        unset($postdata[$key]);
        continue;
      }
  ...
  protected function _save()
  {
    ...
    if (!$this->primary_val) {
      ...
      if (isset($this->pass_var["create"])) {
        foreach ($this->pass_var["create"] as $field => $param) {
          ...
          $postdata[$field] = $this->replace_text_variables(
            $param["value"],
            $postdata
          );
      ...
      if ($this->alert_create) {
        foreach ($this->alert_create as $alert) {
          ...
          $alert["message"] = $this->replace_text_variables(
            $alert["message"],
            $postdata
          );
          ...
          $this->send_email(
            ...
            $message,
            $alert["cc"],
            Xcrud_config::$email_enable_html
          );
        }
      }
      if ($this->mass_alert_create) {
        foreach ($this->mass_alert_create as $alert) {
          if (
            $alert["field"] &&
            isset($postdata[$alert["field"]]) &&
            $postdata[$alert["field"]] != $alert["value"]
          ) {
            continue;
          }
          $alert["message"] = $this->replace_text_variables(
          ...
          $alert["where"] = $this->replace_text_variables(
            $alert["where"],
            $postdata
          );
          if (Xcrud_config::$email_enable_html) {
          ...
          $db = Xcrud_db::get_instance($this->connection);
          $db->query(
            "SELECT `{$alert["email_column"]}` FROM `{$alert["email_table"]}`" .
              ($alert["where"] ? " WHERE " . $alert["where"] : "")
          );
          foreach ($db->result() as $row) {
            $this->send_email(
              $row[$alert["email_column"]],
              $alert["subject"],
              $message,
              [],
              Xcrud_config::$email_enable_html
      ...
      if ($this->before_insert) {
        $path = $this->check_file(
          $this->before_insert["path"],
          "before_insert"
        );
        include_once $path;
        if (is_callable($this->before_insert["callable"])) {
          call_user_func_array($this->before_insert["callable"], [$pd, $this]);
      ...
      if ($this->replace_insert) {
        $path = $this->check_file(
          $this->replace_insert["path"],
          "replace_insert"
        );
        include_once $path;
        if (is_callable($this->replace_insert["callable"])) {
          $this->primary_val = call_user_func_array(
            $this->replace_insert["callable"],
            [$pd, $this]
          );
          $postdata = $pd->to_array();
          if ($this->exception) {
      ...
      if ($this->after_insert) {
        ...
        if (is_callable($this->after_insert["callable"])) {
          call_user_func_array($this->after_insert["callable"], [
            $pd,
            $this->primary_val,
            $this,
          ]);
      ...
      if ($this->send_external_create) {
        if (
          !$this->send_external_create["where_field"] or
          $postdata[$this->send_external_create["where_field"]] ==
            $this->send_external_create["where_val"]
        ) {
          foreach ($this->send_external_create["data"] as $key => $value) {
            $this->send_external_create["data"][
              $key
            ] = $this->replace_text_variables(
              $value,
              $postdata + [
          ...
          switch ($this->send_external_create["method"]) {
            case "include":
              $path = $this->check_file(
                $this->send_external_create["path"],
            ...
            case "post":
              $this->send_http_request(
                $this->send_external_create["path"],
                $this->send_external_create["data"],
                $this->send_external_create["method"],
                false
              );
              break;
          }
    ...
    ON `{$param["table"]}`.`{$param["field"]}` = `{$alias}`.`{$param["join_field"]}`";
      ...
      if ($this->before_update) {
        $path = $this->check_file(
          $this->before_update["path"],
          "before_update"
        );
        include_once $path;
        if (is_callable($this->before_update["callable"])) {
          call_user_func_array($this->before_update["callable"], [
            $pd,
            $this->primary_val,
            $this,
          ]);
      ...
      if ($this->replace_update) {
        $path = $this->check_file(
          $this->replace_update["path"],
          "replace_update"
        );
        include_once $path;
        if (is_callable($this->replace_update["callable"])) {
          $this->primary_val = call_user_func_array(
            $this->replace_update["callable"],
            [$pd, $this->primary_val, $this]
          );
          $postdata = $pd->to_array();
          if ($this->exception) {
            return $this->call_exception($postdata);
      ...
      if ($this->after_update) {
        ...
        if (is_callable($this->after_update["callable"])) {
          call_user_func_array($this->after_update["callable"], [
            $pd,
            $this->primary_val,
            $this,
          ]);
      ...
      if ($this->send_external_edit) {
        if (
          !$this->send_external_edit["where_field"] or
          $postdata[$this->send_external_edit["where_field"]] ==
            $this->send_external_edit["where_val"]
        ) {
          foreach ($this->send_external_edit["data"] as $key => $value) {
            $this->send_external_edit["data"][
              $key
            ] = $this->replace_text_variables($value, $postdata);
          }
          switch ($this->send_external_edit["method"]) {
            case "include":
              $path = $this->check_file(
                $this->send_external_edit["path"],
                "send_external_edit"
              );
              ob_start();
            ...
            case "post":
              $this->send_http_request(
                $this->send_external_edit["path"],
                $this->send_external_edit["data"],
                $this->send_external_edit["method"],
                false
              );
              break;
          }
  ...
  protected function validate_postdata($postdata)
  {
    foreach ($postdata as $key => $val) {
      if (
        isset($this->validation_required[$key]) &&
        mb_strlen($val) < $this->validation_required[$key]
      ) {
        ...
        switch ($this->validation_pattern[$key]) {
          case "email":
            $reg =
              '/^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/u';
            break;
          case "alpha":
            $reg = '/^([a-z])+$/ui';
            break;
  ...
  protected function call_exception($postdata = [])
  {
    ...
    switch ($this->task) {
      case "upload":
        switch ($this->_post("type")) {
          case "image":
            return $this->create_image($this->_post("field"), "") .
              $this->render_message();
            break;
          case "video":
            return $this->create_video($this->_post("field"), "") .
              $this->render_message();
            break;
          case "file":
            return $this->create_file($this->_post("field"), "") .
              $this->render_message();
            break;
          default:
  ...
  {
    if ($message) {
      $this->message = ["type" => $type, "text" => $this->lang($message)];
    }
    if ($fields) {
  ...
  protected function _list()
  {
    ...
    if ($this->sum) {
      foreach ($this->sum as $field => $param) {
        if (isset($this->subselect[$field])) {
          $sum_tmp[$field] =
            "SUM(" . $this->subselect_where($field) . ") AS `" . $field . "`";
        } else {
          $sum_tmp[$field] =
            "SUM(`" .
            $param["table"] .
    ...
    if ($this->before_list) {
      ...
      if (is_callable($this->before_list["callable"])) {
        call_user_func_array($this->before_list["callable"], [
          $this->result_list,
          $this,
        ]);
      }
  ...
  protected function where_pri(
    ...
    if ($fields !== false && $where_val !== false) {
      ...
      if ($index) {
        $this->where_pri[$index] = ["custom" => $fields, "glue" => $glue];
      } else {
        $this->where_pri[] = ["custom" => $fields, "glue" => $glue];
      }
      unset($where_val);
    }
    return $this;
      ...
      if ($val) {
        ...
        if (isset($this->subselect[$field_index])) {
          ...
          if (is_array($this->relation[$field_index]["rel_name"])) {
            ...
            $columns[] =
              "(SELECT GROUP_CONCAT(DISTINCT (CONCAT_WS('{$this->relation[$field_index]["rel_separator"]}'," .
              implode(",", $tmp_fields) .
              ")) SEPARATOR ', ') 
        ...
        elseif (isset($this->fk_relation[$field_index])) {
          ...
          $columns[] =
            ...
            $fk["field"] .
            "` AND " .
            $this->_build_rel_where($field_index) .
            ')
                         AS `' .
            $fk["alias"] .
            "` " .
  ...
  protected function _build_where()
  {
        ...
        if (!isset($params["custom"])) {
          ...
          if (is_array($params["value"])) {
            ...
            if (isset($this->subselect[$fieldkey])) {
              $where_arr[] =
                $this->subselect_where($fieldkey) .
                $this->_cond_from_where_in($params["field"]) .
                "(" .
                implode(",", $in_arr) .
                ")";
            } else {
              $where_arr[] =
                $this->_where_field($params) .
                $this->_cond_from_where_in($params["field"]) .
                "(" .
                implode(",", $in_arr) .
                ")";
            ...
            if (isset($this->subselect[$fieldkey])) {
              $where_arr[] =
                $this->subselect_where($fieldkey) .
                $this->_cond_from_where($params["field"]) .
                $db->escape(
                  $params["value"],
    ...
    if (
      ...
      if ($this->column && isset($search_columns[$this->column])) {
        ...
        elseif (isset($this->subselect[$this->column])) {
          $where_arr[] =
            "(" .
            $this->subselect_query[$this->column] .
            ") LIKE " .
            ...
            case "time":
              ...
              if ($this->phrase["from"] && $this->phrase["to"]) {
                $where_arr[] =
                  ...
                  $fitem["field"] .
                  "` BETWEEN " .
                  $db->escape(gmdate($format, (int) $this->phrase["from"])) .
                  " AND " .
                  $db->escape(gmdate($format, (int) $this->phrase["to"])) .
                  ")";
            ...
            if ($value != "") {
              ...
              if ($condition == "LIKE") {
                $where_arr[] =
                  ...
                  $field .
                  "`))LIKE " .
                  $db->escape_like($value, $this->search_pattern);
              } elseif ($condition == "IN") {
                $where_arr[] =
                  "CONCAT(X(`" .
                  $table .
                  ...
                  case "time":
                    ...
                    if ($value != "") {
                      ...
                      $where_arr[] =
                        ...
                        $condition .
                        " " .
                        $db->escape(gmdate($format, (int) $value)) .
                        ")";
                    }
        ...
        //print_r($this->advanced);
        $advanced_params =
          "<div style='color:#931010;font-size:16px;'>" .
          $this->theme_config("search_advanced_title") .
          " <br>";
            ...
            foreach ($this->advanced_filter as $field_ => $fitem_) {
              if ($field_ == $field_spe) {
                ...
                if (Xcrud_config::$load_semantic) {
                  $advanced_params .=
                    "<span data='$field' class='search_values'>" .
                    $label .
                    " (" .
                    $field_array_spe[3] .
                    ") " .
                    $value_ .
                    "<button class='btn button btn_search_values'><i class='window close icon'></i></button></span><br>";
                } elseif (Xcrud_config::$load_bootstrap) {
                  $advanced_params .=
                    "<span data='$field' class='search_values'>" .
                    $label .
                    " (" .
                    ...
                    ") " .
                    $value_ .
                    "<button class='btn button btn_search_values'><i class='fa fa-close'></i></button></span><br>";
                } elseif (Xcrud_config::$load_bootstrap4) {
                  $advanced_params .=
                    "<span data='$field' class='search_values'>" .
                    $label .
                    " (" .
                    ...
                    $value_ .
                    "<button class='btn button btn_search_values'><i class='fa fa-close'></i></button></span><br>";
                } elseif (Xcrud_config::$load_bootstrap5) {
                  $advanced_params .=
                    "<span data='$field' class='search_values'>" .
                    $label .
                    " (" .
                  ...
                  $advanced_params .=
                    ...
                    ") " .
                    $value_ .
                    "<button class='btn button btn_search_values'><i class='icon-close window close icon fa fa-close'></i></button></span><br>";
                }
              }
            }
          }
        ...
        foreach ($search_columns as $key => $fitem) {
          ...
          if (isset($this->relation[$key])) {
            ...
            $this->field_type[$key] == "date" ||
            $this->field_type[$key] == "datetime" ||
            $this->field_type[$key] == "timestamp" ||
            $this->field_type[$key] == "time"
          ) {
    ...
    } elseif (empty($where_arr) && !empty($where_arr_pri)) {
      return "WHERE " .
        ($where_arr_pri
          ? ($where_arr ? " AND " : "") . implode(" ", $where_arr_pri)
          : "");
    } elseif (!empty($where_arr) && !empty($where_arr_pri)) {
      return "WHERE " .
        ($where_arr ? "(" . implode(" ", $where_arr) . ")" : "") .
  ...
  protected function _build_relation_subwhere($key)
  {
    // multicolumn name
    $db = Xcrud_db::get_instance($this->connection);
    ...
    if ($key) {
      ...
      if (is_array($rel["rel_name"])) {
        ...
        $select =
          "(SELECT GROUP_CONCAT(DISTINCT (CONCAT_WS('{$rel["rel_separator"]}'," .
          implode(",", $tmp_fields) .
          ")) SEPARATOR ', ') 
                            FROM `{$rel["rel_tbl"]}` 
                            WHERE {$where})\r\n";
  ...
  /** relation values will be searched by displayed name (not by id) */
  protected function _build_relation_subwhere_condition($key, $condition)
  {
    // multicolumn name
    $db = Xcrud_db::get_instance($this->connection);
      ...
      if (is_array($this->fk_relation[$name]["rel_where"])) {
        foreach ($this->fk_relation[$name]["rel_where"] as $field => $val) {
          $val = preg_replace_callback(
            "/\{(.+)\}/Uu",
            [$this, "rel_where_callback"],
            $val
          );
          $where_arr[] =
            $this->_field_from_where($field) .
            $this->_cond_from_where($field) .
            $db->escape($val);
        }
      } else {
        $where_arr[] = preg_replace_callback(
          "/\{(.+)\}/Uu",
          [$this, "rel_where_callback"],
  ...
  protected function _receive_post($task = false, $primary = false)
  {
    if (!$this->table_name && !$this->query) {
      $this->table_name = $this->_humanize(
        mb_substr($this->table, mb_strlen($this->prefix))
      );
    }
    if ($task) {
      switch ($task) {
    ...
    if ($this->is_get) {
      ...
      if ($this->order_column) {
        if (!$this->query) {
          $this->order_column = key(
            $this->_parse_field_names(
              $this->order_column,
              "receive_post",
              false,
              false
      ...
      $this->limit = $this->_post(
        "limit",
        $this->limit ? $this->limit : Xcrud_config::$limit
      );
    ...
    if (count($this->order_by)) {
      ...
      foreach ($this->order_by as $field => $direction) {
        if ($direction === false) {
          ...
          isset($this->subselect[$field]) or
          isset($this->columns[$field]) or
          isset($this->no_select[$field]) or
          !strpos($field, ".") or
          isset($this->fk_relation[$field])
        ) {
    ...
    if (is_array($this->table_info) && count($this->table_info)) {
      ...
      foreach ($this->table_info as $table => $types) {
        foreach ($types as $row) {
          ...
          $this->field_null[$field_index] =
            $row["Null"] == "YES" ? true : false;
          if (
            !$this->field_null[$field_index] &&
            Xcrud_config::$not_null_is_required &&
            !isset($this->validation_required[$field_index])
          ) {
          ...
          if ($row["Key"] == "PRI" && $row["Extra"] == "auto_increment") {
            ...
            if (
              (!$this->show_primary_ai_column && $mode == "list" or
                !$this->show_primary_ai_field && $mode != "list") &&
          ...
          if (
            $this->join &&
            isset($this->join[$table]) &&
            $this->join[$table]["join_field"] == $row["Field"]
          ) {
            if (!isset($this->field_type[$field_index])) {
              $this->_define_field_type($row, $field_index);
            }
          ...
          if (!$fields_object) {
            ...
            $fields_object &&
            isset($this->reverse_fields[$mode]) &&
            $this->reverse_fields[$mode]
          ) {
      ...
      if (
        $fields_object &&
        (!isset($this->reverse_fields[$mode]) || !$this->reverse_fields[$mode])
      ) {
          ...
          switch ($this->task) {
            ...
            default:
              self::error(
                '<strong>Table "' .
                  $this->table .
                  '" has no any primary or unique key!</strong><br />
  ...
  protected function _define_field_type($row, $field_index)
  {
    ...
    if (!isset($this->field_attr[$field_index])) {
      $this->field_attr[$field_index] = [];
    }
    if (
      $max_l &&
      (!isset($this->field_attr[$field_index]["maxlength"]) or
    ...
    switch ($type) {
      ...
      case "longtext":
        ...
        if (
          !isset($this->no_editor[$field_index]) &&
          Xcrud_config::$auto_editor_insertion
        ) {
      ...
      case "enum":
        ...
        $this->field_type[$field_index] = Xcrud_config::$enum_as_radio
          ? "radio"
          : "select";
      ...
      case "set":
        ...
        $this->field_type[$field_index] = Xcrud_config::$set_as_checkboxes
          ? "checkboxes"
          : "multiselect";
        $this->field_attr[$field_index]["values"] = $max_l;
        if (!isset($this->defaults[$field_index])) {
  ...
  protected function _set_column_names()
  {
    ...
    foreach ($this->columns as $key => $col) {
      ...
      if (isset($this->column_name[$key])) {
        ...
        $this->columns_names[$key] = $this->html_safe(
          $this->_humanize($col["field"])
        );
      }
    }
    if ($subselect_before) {
  ...
  protected function _set_field_names()
  {
    foreach ($this->fields as $key => $field) {
      if (isset($this->labels[$key])) {
        $this->fields_names[$key] =
          $this->html_safe($this->labels[$key]) .
          (isset($this->validation_required[$key]) ? "&#42;" : "");
      } else {
        $this->fields_names[$key] =
          $this->html_safe($this->_humanize($field["field"])) .
          (isset($this->validation_required[$key]) ? "&#42;" : "");
      }
    }
  }
  ...
  protected function _render_details($mode)
  {
    ...
    if ($this->result_row) {
      foreach ($this->fields as $field => $fitem) {
        ...
        if ($this->field_type[$field] == "hidden") {
          $this->hidden_fields_output[$field] = $this->create_hidden(
            $field,
          ...
          if (isset($this->field_callback[$field]) && $mode != "view") {
            $path = $this->check_file(
              $this->field_callback[$field]["path"],
              "field_callback"
            );
            include_once $path;
            if (is_callable($this->field_callback[$field]["callback"])) {
              $this->fields_output[$field] = [
            ...
            $path = $this->check_file(
              $this->column_callback[$field]["path"],
              "column_callback"
            );
            include_once $path;
            if (
              is_callable($this->column_callback[$field]["callback"]) &&
              $this->result_row
            ) {
              $this->fields_output[$field] = [
                "label" => $this->fields_names[$field],
            ...
            $this->fields_output[$field] = [
              ...
              "field" => call_user_func_array(
                [$this, $func],
                [$field, $this->result_row[$field], $attr]
              ),
              "name" => $field,
              "value" => $this->result_row[$field],
            ];
            if (isset($this->column_pattern[$field]) && $mode == "view") {
              $this->fields_output[$field]["field"] = str_ireplace(
                "{value}",
                $this->fields_output[$field]["field"],
                $this->column_pattern[$field]
              );
              $this->fields_output[$field][
                "field"
              ] = $this->replace_text_variables(
    ...
    if ($this->inner_table_instance && ($mode == "view" or $mode == "edit")) {
        ...
        if (isset($this->result_row[$field])) {
          ...
          if ($instance->inner_table_template != "") {
            ...
            $file =
              dirname(__FILE__) .
              "/../../pages/" .
              $instance->inner_table_template;
            include "$file";
            $custom_page = ob_get_clean();
            $this->nested_rendered[$inst_name] =
              //$custom_page;
              '<div class="xcrud-nested-container xcrud-container"><div class="xcrud-ajax" id="xcrud-ajax-' .
  ...
  protected function _pagination(
    ...
    if ($total > $limit) {
      ...
      $html =
        '<ul class="' . $this->theme_config("pagination_container") . '">';
      if ($pages > $numpos) {
        if ($curent <= $numlr + 3) {
          ...
          $html .=
            '<li class="' .
            $this->theme_config("pagination_dots") .
            '"><span class="' .
            $this->theme_config("pagination_link") .
            '">&#133;</span></li>';
          for ($i = $pages - $numlr + 1; $i <= $pages; ++$i) {
            $html .= $this->_pagination_item($i, $curent, $limit);
          }
        } elseif ($curent >= $pages - $numlr - 2) {
          for ($i = 1; $i <= $numlr; ++$i) {
            $html .= $this->_pagination_item($i, $curent, $limit);
          }
          $html .=
  ...
  protected function _next_previous(
    $total,
    $start,
    $limit,
    $numpos = 10,
  ...
  protected function _pagination_item($i, $curent, $limit)
  {
    ...
    if ($i == $curent) {
      return '<li class="' .
        $this->theme_config("pagination_active") .
        '"><span class="' .
        $this->theme_config("pagination_link") .
        '">' .
        $i .
      ...
      return '<li class="' .
        ...
        '"><a href="javascript:;" class="xcrud-action page-link" data-start="' .
        $limit1 .
        '">' .
        $i .
        "</a></li>";
  ...
  protected function _cut($string, $field, $wordsafe = true, $dots = true)
  {
    ...
    if (
      $slen <= $len ||
      (Xcrud_config::$print_full_texts && $this->theme == "printout")
    ) {
      return $this->output_string($string, $this->strip_tags, $safe);
    }
    if ($wordsafe) {
      $end = $len;
      while (
        mb_substr($strip_string, --$len, 1, Xcrud_config::$mbencoding) != " " &&
        $len > 0
      ) {
      }
      if ($len == 0) {
      ...
      return $this->output_string(
        mb_substr($strip_string, 0, $len, Xcrud_config::$mbencoding),
        false,
        $safe
      ) . ($dots ? "&#133;" : "");
    }
  ...
  public function _export_vars()
  {
    ...
    if ($this->instance_count == 1 && !$this->ajax_request) {
      if (
        isset($_SESSION["lists"]["xcrud_session"]) &&
        $_SESSION["lists"]["xcrud_session"]
      ) {
    ...
    if (Xcrud_config::$alt_session) {
      ...
      if (class_exists("Memcache")) {
        ...
        self::error(
          'Can\'t use alternative session. Memcache(d) is not available'
        );
      }
      unset($_SESSION["lists"]["xcrud_session"]);
      if (!$res) {
        self::error(
          'Can\'t use alternative session. Memcache(d) has invalid parameters or broken. Storing failed'
        );
      }
    }
  }
    ...
    foreach ($this->params2save() as $item) {
      try {
        if (isset($_SESSION["lists"]["xcrud_session"][$inst_name][$item])) {
          $this->{$item} =
            $_SESSION["lists"]["xcrud_session"][$inst_name][$item];
        }
      } catch (exception $e) {
      }
    }
  ...
  protected function get_field_attr($name, $mode)
  {
    ...
    if (
      isset($this->relation[$name]["depend_on"]) &&
      $this->relation[$name]["depend_on"]
    ) {
      $tag["data-depend"] = $this->relation[$name]["depend_on"];
    }
    return $tag;
  }
  protected function create_none($name, $value = "", $tag = [])
  {
  ...
  protected function create_button($name, $value = "")
  {
    ...
    return '<button val="' .
      $value .
      '" type="button" class="btn btn-primary" onclick=Xcrud.request(".xcrud-ajax",Xcrud.list_data(".xcrud-ajax",{task:"save",after:"edit"}));>' .
      $this->labels[$name] .
      "</button>";
  }
  protected function create_save_and_edit($name, $value = "")
  {
    ...
    return '<button val="' .
      $value .
      '" type="button" class="btn btn-primary" onclick=Xcrud.request(".xcrud-ajax",Xcrud.list_data(".xcrud-ajax",{task:"save",after:"edit"}),false,3);>' .
      $this->labels[$name] .
      "</button>";
  }
  protected function create_save_and_return($name, $value = "")
  {
    ...
    return '<button val="' .
      $value .
      '" type="button" class="btn btn-primary" onclick=Xcrud.request(".xcrud-ajax",Xcrud.list_data(".xcrud-ajax",{task:"save",after:"list"}),false,3);>' .
      $this->labels[$name] .
      "</button>";
  }
  protected function create_make_payment($name, $value = "")
  {
    ...
    return '<button val="' .
      $value .
      '" type="button" class="btn btn-primary" onclick=makePayment();>' .
      $this->labels[$name] .
      "</button>";
  }

  protected function create_bool($name, $value = "", $tag = [])
  {
    ...
    return $this->open_tag("div", $this->theme_config("checkbox_container")) .
      $this->open_tag("label") .
      $this->single_tag(
        $tag,
      ...
      $this->close_tag("label") .
      $this->close_tag("div");
  }
  protected function create_view_bool($name, $value = "", $tag = [])
  {
    return (int) $value ? $this->lang("bool_on") : $this->lang("bool_off");
  }
  protected function create_int($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
    ...
    return $this->single_tag(
      $tag,
      $this->theme_config("int_field"),
      $int_val,
      true
    );
  }
  protected function create_view_int($name, $value = "", $tag = [])
  {
    return $value;
  }
  protected function create_float($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
    ...
    return $this->single_tag(
      $tag,
      $this->theme_config("float_field"),
      $float_val,
      true
    );
  }
  protected function create_view_float($name, $value = "", $tag = [])
  {
    return $value;
  }
  protected function create_price($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
    ...
    return $this->single_tag(
      $tag,
      $this->theme_config("price_field"),
      $this->field_attr[$price_val],
      true
    );
  }

  protected function create_view_price($name, $value = "", $tag = [])
  {
    $out = "";
  ...
  protected function create_text($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
    ...
    return $this->single_tag(
      $tag,
      $this->theme_config("text_field"),
      $text_val,
      true
    );
  }
  protected function create_view_text($name, $value = "", $tag = [])
  {
  ...
  protected function create_textarea($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
    ...
    return $this->open_tag(
      $tag,
      $this->theme_config("textarea_field"),
      $textarea_val,
      true
    ) .
      $this->html_safe($value) .
      $this->close_tag($tag);
  }
  protected function create_view_textarea($name, $value = "", $tag = [])
  {
    return $value;
  }
  protected function create_texteditor($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
      return $this->create_view_texteditor($name, $value, $tag = []);
    }
    $tag = $tag + [
      "tag" => "textarea",
    ...
    return $this->open_tag(
      $tag,
      $this->theme_config("texteditor_field"),
      $texteditor_val,
      true
    ) .
      $this->html_safe($value) .
  ...
  protected function create_view_texteditor($name, $value = "", $tag = [])
  {
    return $value;
  }
  protected function create_date($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
      return $this->create_view_date($name, $value, $tag = []);
    }
    $tag = $tag + [
      "tag" => "input",
      "type" => "text",
    ...
    return $this->single_tag(
      $tag,
      $this->theme_config("date_field"),
      $date_val,
      true
    );
  }
  protected function create_view_date($name, $value = "", $tag = [])
  {
    return $this->mysql2date($value);
  }
  protected function create_datetime($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
      return $this->create_view_datetime($name, $value, $tag = []);
    }
    $tag = $tag + [
      "tag" => "input",
      "type" => "text",
    ...
    return $this->single_tag(
      $tag,
      $this->theme_config("datetime_field"),
      $datetime_val,
      true
    );
  }
  protected function create_view_datetime($name, $value = "", $tag = [])
  {
    return $this->mysql2datetime($value);
  }
  protected function create_timestamp($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
      return $this->create_view_timestamp($name, $value, $tag = []);
    }
    $tag = $tag + [
      "tag" => "input",
      "type" => "text",
    ...
    return $this->single_tag(
      $tag,
      $this->theme_config("timestamp_field"),
      $timestamp_val,
      true
    );
  }
  protected function create_view_timestamp($name, $value = "", $tag = [])
  {
    return $this->mysql2datetime($value);
  }
  protected function create_time($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
    ...
    return $this->single_tag(
      $tag,
      $this->theme_config("time_field"),
      $time_val,
      true
    );
  }
  protected function create_view_time($name, $value = "", $tag = [])
  {
    return $this->mysql2time($value);
  }
  protected function create_year($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
    ...
    return $this->single_tag(
      $tag,
      $this->theme_config("year_field"),
      $year_val,
      true
    );
  }
  protected function create_view_year($name, $value = "", $tag = [])
  {
    return $value;
  }
  protected function create_select($name, $value = "", $tag = [])
  {
    ...
    $out .= $this->open_tag(
      $tag,
      $this->theme_config("select_field"),
      $this->field_attr[$name],
      true
    );
    ...
    if (is_array($this->field_attr[$name]["values"])) {
      foreach ($this->field_attr[$name]["values"] as $optkey => $opt) {
        if (is_array($opt)) {
          $out .= $this->open_tag(["tag" => "optgroup", "label" => $optkey]);
          foreach ($opt as $k_key => $k_opt) {
            $opt_tag = ["tag" => "option", "value" => $k_key];
            if ($k_key == $value) {
              $opt_tag["selected"] = "";
            }
            $out .=
              $this->open_tag($opt_tag) .
              $this->html_safe($k_opt) .
              $this->close_tag($opt_tag);
          }
          $out .= $this->close_tag("optgroup");
        } else {
          $opt_tag = ["tag" => "option", "value" => $optkey];
          if ($optkey == $value) {
            $opt_tag["selected"] = "";
          }
          $out .=
  ...
  protected function create_radio($name, $value = "", $tag = [])
  {
    $checkName = explode(".", $name);
    $cntName = count($checkName);
    $name_old = "";
    ...
    $label_tag = ["tag" => "label", "class" => "xcrud-radio-label"];

    if (is_array($this->field_attr[$name]["values"])) {
      foreach ($this->field_attr[$name]["values"] as $optkey => $opt) {
        $out .=
          $this->open_tag("div", $this->theme_config("radio_container")) .
          $this->open_tag($label_tag);
        $attr = ["value" => $optkey];
        if ($optkey == $value) {
          $attr["checked"] = "";
        }
        $out .= $this->single_tag(
          $tag,
          $this->theme_config("radio_field"),
          array_merge($this->field_attr[$name], $attr),
          true
        );
        $out .=
          $this->html_safe($opt) .
          $this->close_tag($label_tag) .
          $this->close_tag("div");
      }
    } else {
      $tmp = $this->parse_comma_separated($this->field_attr[$name]["values"]);
  ...
  protected function create_view_select($name, $value = "", $tag = [])
  {
    if (is_array($this->field_attr[$name]["values"])) {
  ...
  protected function create_view_radio($name, $value = "", $tag = [])
  {
    return $this->create_view_select($name, $value, $tag);
  }
  protected function create_multiselect($name, $value = "", $tag = [])
  {
    $checkName = explode(".", $name);
    $cntName = count($checkName);
    $name_old = "";
    ...
    $out .= $this->open_tag(
      $tag,
      $this->theme_config("multiselect_field"),
      $this->field_attr[$name],
      true
    );
  ...
  protected function create_checkboxes($name, $value = "", $tag = [])
  {
    $checkName = explode(".", $name);
    $cntName = count($checkName);
    $name_old = "";
    ...
    $label_tag = ["tag" => "label", "class" => "xcrud-checkboxes-label"];

    if (is_array($this->field_attr[$name]["values"])) {
      foreach ($this->field_attr[$name]["values"] as $optkey => $opt) {
        $out .=
          $this->open_tag("div", $this->theme_config("checkbox_container")) .
          $this->open_tag($label_tag);
        $attr = ["value" => $optkey];
        if (in_array($optkey, $values)) {
          $attr["checked"] = "";
        ...
        $out .= $this->single_tag(
          $tag,
          $this->theme_config("checkboxes_field"),
          array_merge($this->field_attr[$name], $attr),
          true
        );
        $out .=
  ...
  protected function create_view_multiselect($name, $value = "", $tag = [])
  {
    $out = [];
    $values = $this->parse_comma_separated($value);
  ...
  protected function create_view_checkboxes($name, $value = "", $tag = [])
  {
    return $this->create_view_multiselect($name, $value, $tag);
  }
  protected function create_hidden($name, $value = "", $tag = [])
  {
    return $this->single_tag(
      $tag + [
        "tag" => "input",
  ...
  protected function create_password($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
    ...
    return $this->single_tag(
      $tag,
      $this->theme_config("password_field"),
      $this->field_attr[$name],
      true
    );
  }
  protected function create_view_password($name, $value = "", $tag = [])
  {
    return "*****";
  }
  protected function create_relation(
    $name,
    $value = "",
    $tag = [],
    $dependval = false
      ...
      if (is_array($this->relation[$name]["rel_where"])) {
        foreach ($this->relation[$name]["rel_where"] as $field => $val) {
          ...
          $fdata = $this->_parse_field_names(
            $field,
            "create_relation",
            $this->relation[$name]["rel_tbl"]
          );
          $fitem = reset($fdata);
          $where_arr[] =
            $this->_where_field($fitem) .
            $this->_cond_from_where($field) .
            $db->escape($val);
        }
      } else {
        $where_arr[] = $this->replace_text_variables(
          $this->relation[$name]["rel_where"],
          $this->result_row
        );
      }
    }
    if ($dependval !== false) {
      $where_arr[] =
        $this->_field_from_where($this->relation[$name]["depend_field"]) .
        $this->_cond_from_where($this->relation[$name]["depend_field"]) .
        $db->escape($dependval);
    }
    $out .= $this->open_tag(
      $tag,
      $this->theme_config("relation_field"),
      $this->field_attr[$name],
      true
    );
    ...
    if ($this->relation[$name]["depend_on"] && $dependval === false) {
      ...
      if ($this->lists_null_opt) {
        foreach ($values as $val) {
          $out .=
            ...
            $this->lang("null_option") .
            $this->close_tag("option");
        }
      }
    } else {
      if ($where_arr) {
      ...
      if (is_array($this->relation[$name]["rel_name"])) {
        $name_select =
          "CONCAT_WS(" .
          $db->escape($this->relation[$name]["rel_separator"]) .
          ",`" .
          implode("`,`", $this->relation[$name]["rel_name"]) .
          "`) AS `name`";
      } else {
      ...
      $db->query(
        "SELECT `" .
          $this->relation[$name]["rel_field"] .
          "` AS `field`," .
          $name_select .
          $this->get_relation_tree_fields($this->relation[$name]) .
          " FROM `" .
          $this->relation[$name]["rel_tbl"] .
          "` " .
          $where .
          " GROUP BY `field` ORDER BY " .
          $this->get_relation_ordering($this->relation[$name])
      );
      $options = $this->resort_relation_opts(
        $db->result(),
        $this->relation[$name]
      );
      if ($this->lists_null_opt) {
        $out .=
          $this->open_tag(["tag" => "option", "value" => ""]) .
          $this->lang("null_option") .
          $this->close_tag("option");
      }
    }
    if ($options) {
      foreach ($options as $opt) {
        $opt_tag = ["tag" => "option", "value" => $opt["field"]];
        if (in_array($opt["field"], $values)) {
          $opt_tag["selected"] = "";
        }
        $out .=
          $this->open_tag($opt_tag) .
          $this->html_safe($opt["name"]) .
          $this->close_tag($opt_tag);
      }
    }
    $out .= $this->close_tag($tag);
  ...
  protected function create_view_relation(
    $name,
    $value = "",
    $tag = [],
    $dependval = false
  ...
  protected function get_relation_ordering($rel)
  {
    if (
      $rel["tree"] &&
      isset($rel["tree"]["left_key"]) &&
      isset($rel["tree"]["level_key"])
    ) {
      return "`" . $rel["tree"]["left_key"] . "` ASC";
    } elseif (
      $rel["tree"] &&
      isset($rel["tree"]["parent_key"]) &&
      isset($rel["tree"]["primary_key"])
    ) {
  ...
  protected function create_fk_relation($name, $value = "", $tag = [])
  {
    ...
    if (is_array($this->fk_relation[$name]["rel_name"])) {
      ...
      foreach ($this->fk_relation[$name]["rel_name"] as $optnms) {
        $optnames[] =
          "`" . $this->fk_relation[$name]["rel_tbl"] . "`.`" . $optnms . "`";
      }
      $name_select =
        "CONCAT_WS(" .
        $db->escape($this->fk_relation[$name]["rel_separator"]) .
        "," .
        implode(",", $optnames) .
        ") AS `name`";
    } else {
      $name_select =
        ...
        $this->fk_relation[$name]["rel_tbl"] .
        "`.`" .
        $this->fk_relation[$name]["rel_name"] .
        "` AS `name`";
    }
    ...
    $db->query(
      "SELECT `" .
        $this->fk_relation[$name]["rel_tbl"] .
        "`.`" .
        $this->fk_relation[$name]["rel_field"] .
        "` AS `field`," .
        $name_select .
        " FROM `" .
        $this->fk_relation[$name]["rel_tbl"] .
  ...
  protected function create_view_fk_relation($name, $value = "", $tag = [])
  {
    if (!isset($this->fk_relation[$name])) {
      return "Restricted.";
    }
  ...
  protected function create_file(
    ...
    if ($value) {
      ...
      $attr = [
        "href" => isset($this->upload_config[$name]["url"])
          ? $this->real_file_link($value, $this->upload_config[$name], true)
      ...
      $out .= isset($this->upload_config[$name]["text"])
        ? $this->upload_config[$name]["text"]
        : $value;
      ...
      $out .= $this->open_tag(
        "span",
        "xcrud-nofile " . $this->theme_config("no_file")
      );
  ...
  protected function upload_file_button($name, $value, $tag = [])
  {
    $out = "";
    $out .= $this->open_tag("span", $this->theme_config("upload_button"), [
      "class" => "xcrud-add-file",
    ]);
    if (!$this->is_rtl && $this->theme_config("upload_button_icon")) {
      $out .=
        $this->open_tag("i", $this->theme_config("upload_button_icon")) .
        $this->close_tag("i") .
        " ";
    }
  ...
  protected function upload_image_button($name, $value, $tag = [])
  {
    $out = "";
    $out .= $this->open_tag("span", $this->theme_config("upload_button"), [
      "class" => "xcrud-add-file",
  ...
  protected function upload_video_button($name, $value, $tag = [])
  {
    $out = "";
    $out .= $this->open_tag("span", $this->theme_config("upload_button"), [
      "class" => "xcrud-add-file",
  ...
  protected function remove_upload_button($name)
  {
    ...
    if (!$this->is_rtl && $this->theme_config("remove_button_icon")) {
      $out .=
        $this->open_tag("i", $this->theme_config("remove_button_icon")) .
        $this->close_tag("i") .
        " ";
    }
  ...
  protected function create_view_file(
    $name,
    $value = "",
    $tag = [],
    $is_upload = false
    ...
    if ($value) {
      ...
      $out .=
        (isset($this->upload_config[$name]["text"])
          ? $this->upload_config[$name]["text"]
          : $value) . $this->close_tag("a");
      $out .= " " . $file_size;
      $this->close_tag("span");
    } else {
  ...
  protected function create_image(
    ...
    if (isset($this->subselect[$name])) {
      return $this->create_view_image($name, $value, $tag = []);
    }
    $out = ""; // upload container
    ...
    if ($value) {
      ...
      $attr = [
        "src" => isset($this->upload_config[$name]["url"])
          ? $this->real_file_link($value, $this->upload_config[$name], true)
          : $this->file_link(
            $name,
            $this->primary_val,
            isset($this->upload_config[$name]["detail_thumb"])
              ? $this->upload_config[$name]["detail_thumb"]
              : false,
            false,
            $value
          ),
      ...
      if (!isset($tag["readonly"]) && !isset($tag["disabled"])) {
        $out .= $this->open_tag(
          "div",
          $this->theme_config("grid_button_group")
        );
        // delete button
        $out .= $this->remove_upload_button($name);
      }
      ...
      $out .= $this->open_tag(
        "span",
        "xcrud-noimage " . $this->theme_config("no_file")
      );
  ...
  protected function create_video(
    $name,
    $value = "",
    $tag = [],
    $is_upload = false
    ...
    if (isset($this->subselect[$name])) {
      return $this->create_view_video($name, $value, $tag = []);
    }
    $out = ""; // upload container
    $out .= $this->open_tag("div", "xcrud-upload-container"); // image and delete button
    if ($value) {
      ...
      $attr_vid = [
        ...
        "id" => "video-js",
        "class" => "video-size video-js vjs-default-skin",
        "preload" => "auto",
        "data-setup" => "{}",
      ];
  ...
  protected function create_view_image($name, $value = "", $tag = [])
  {
  ...
  protected function create_view_video($name, $value = "", $tag = [])
  {
    $out = ""; // image and delete button
    if ($value) {
      /*$binary = isset($this->upload_config[$name]['blob']) ? true : false;
  ...
  protected function create_binary($name, $value = "", $tag = [])
  {
    return $value ? "[binary data]" : "[binary null]";
  }
  protected function create_view_binary($name, $value = "", $tag = [])
  {
    return $value ? "[binary data]" : "[binary null]";
  }
  protected function create_remote_image($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
    ...
    $out .= $this->single_tag(
      $tag,
      $this->theme_config("remote_image_field"),
      $attr,
      true
    );
    return $out;
  }
  protected function create_view_remote_image($name, $value = "", $tag = [])
  {
    if ($value) {
      $attr = $this->field_attr[$name];
      $img = [
  ...
  protected function create_point($name, $value = "", $tag = [])
  {
    if (isset($this->subselect[$name])) {
    ...
    $out .=
      $this->open_tag($map, $this->theme_config("point_map")) .
      $this->close_tag($map);

    return $out;
  }
  protected function create_view_point($name, $value = "", $tag = [])
  {
    $out = "";
    $attr = $this->field_attr[$name];
    if (!$value) {
  ...
  protected function benchmark_end()
  {
    if ($this->benchmark) {
      ...
      $out =
        ...
        $this->lang("exec_time") .
        " " .
        number_format($this->time_end - $this->time_start, 3, ".", "") .
        " s</span>";
      $out .=
        "<span>" .
        $this->lang("memory_usage") .
        " " .
        number_format(
          ($this->memory_end - $this->memory_start) / 1024 / 1024,
          2,
          ".",
          ""
        ) .
  ...
  {
    exit(
      '<div class="xcrud-error" style="position:relative;line-height:1.25;padding:15px;color:#BA0303;margin:10px;border:1px solid #BA0303;border-radius:4px;font-family:Arial,sans-serif;background:#FFB5B5;box-shadow:inset 0 0 80px #E58989;">
            <span style="position:absolute;font-size:10px;bottom:3px;right:5px;">xCRUD</span>' .
        $text .
        "</div>"
    );
  }
  ...
  protected function _upload_file()
  {
    ...
    if (
      isset($_FILES) &&
      isset($_FILES["xcrud-attach"]) &&
      !$_FILES["xcrud-attach"]["error"]
    ) {
      ...
      $out = $this->create_file($field, $filename, [], true);
      $this->after_render();
      return $out;
    } else {
    ...
    if (
      ...
      if ($this->is_resize($field)) {
        ...
        if ($this->is_manual_crop($field)) {
          ...
          $out = $this->create_image($field, $filename, [], true);
        }
      } else {
  ...
  protected function _upload_video()
  {
    ...
    if (
      ...
      $out = $this->create_video($field, $filename, [], true);
  ...
  protected function render_crop_window($filename, $field)
  {
    ...
    $ratio = isset($this->upload_config[$field]["ratio"])
      ? $this->upload_config[$field]["ratio"]
      : "";
    $attr = [
    ...
    /*$out .= $this->single_tag('input', 'new_key', array(
        ...
        'value' => $this->key,
        'type' => 'hidden'));*/
    $out .= $this->single_tag("input", "xrud-crop-data", [
      "name" => "field",
      "value" => $field,
      "type" => "hidden",
    ]);
  ...
  protected function filter_image($filename, $field)
  {
    ...
    $watermark =
      isset($settings["watermark"]) && $settings["watermark"]
        ? $this->check_file($settings["watermark"], "try_crop_image")
        : false;
    $watermark_position =
      isset($settings["watermark_position"]) &&
      is_array($settings["watermark_position"]) &&
      count($settings["watermark_position"] == 2)
        ? array_values($settings["watermark_position"])
        : [50, 50];
    $quality =
      isset($settings["quality"]) && $settings["quality"]
        ? $settings["quality"]
        : 92;
    $this->_draw_watermark(
      $tmp_path,
      $file_path,
      $quality,
      $watermark,
  ...
  protected function manual_crop()
  {
    ...
    $settings = $this->upload_config[$field];
    $ratio =
      isset($settings["ratio"]) && !empty($settings["ratio"])
        ? (float) $settings["ratio"]
        : $w / $h;
    $tmp_path = $this->get_image_folder($field) . "/" . $tmp_name;
    $file_path = $this->get_image_folder($field) . "/" . $filename;
    $this->_try_crop_image(
      $tmp_path,
      $file_path,
      $field,
      $settings,
  ...
  protected function _try_crop_image(
    $tmp_path,
    $file_path,
    $field,
    $settings,
    ...
    $set_w =
      isset($settings["width"]) && !empty($settings["width"])
        ? (int) $settings["width"]
        : false;
    $set_h =
      isset($settings["height"]) && !empty($settings["height"])
        ? (int) $settings["height"]
        : false;
  ...
  {
    $crop =
      isset($settings["crop"]) && $settings["crop"] == true ? true : false;
    $width =
      isset($settings["width"]) && $settings["width"]
        ? $settings["width"]
        : false;
    $height =
      isset($settings["height"]) && $settings["height"]
        ? $settings["height"]
        : false;
    $watermark =
      isset($settings["watermark"]) && $settings["watermark"]
        ? $this->check_file($settings["watermark"], "try_change_size")
        : false;
    $watermark_position =
      isset($settings["watermark_position"]) &&
      is_array($settings["watermark_position"]) &&
  ...
  {
    $type = isset($this->field_type[$this->_post("field")])
      ? $this->field_type[$this->_post("field")]
      : false;
    switch ($type) {
      case "image":
  ...
  protected function _image_resize(
    ...
    if ($srcWidth >= $srcHeight) {
      $ratio =
        ($new_size_w ? $srcWidth : $srcHeight) /
        ($new_size_w ? $new_size_w : $new_size_h);
      $ratio = max($ratio, 1.0);
      ...
      $ratio =
        ($new_size_h ? $srcHeight : $srcWidth) /
        ($new_size_h ? $new_size_h : $new_size_w);
      $ratio = max($ratio, 1.0);
      $destWidth = $srcWidth / $ratio;
      $destHeight = $srcHeight / $ratio;
    ...
    if ($watermark) {
      ...
      $offsets = $this->calculate_watermark_offsets(
        $destWidth,
        $destHeight,
        $water_w,
        $water_h,
    ...
    $newimgarray = [$destWidth, $destHeight];
    return $newimgarray;
  }
  protected function _image_crop(
    $src_file,
    $dest_file,
    $new_size_w,
    $new_size_h,
  ...
  protected function _draw_watermark(
    $src_file,
    $dest_file,
    $dest_qual = 95,
    $watermark = false,
  ...
  protected function calculate_watermark_offsets(
    $img_w,
    $img_h,
    $water_w,
    $water_h,
  ...
  protected function _file_size($path)
  {
    return number_format(
      is_file($path) ? filesize($path) / 1024 : 0,
      2,
      ".",
      " "
    ) . " KB";
  ...
  protected function _where_field($param)
  {
    return "`" .
      $param["table"] .
      "`.`" .
      $this->_field_from_where($param["field"]) .
      "`";
  }
  ...
  protected function _compare($val1, $operator, $val2)
  {
    switch ($operator) {
      ...
      case "!=":
        return $val1 != $val2 ? true : false;
      case "^=":
        return mb_strpos($val1, $val2, 0, Xcrud_config::$mbencoding) === 0
          ? true
          : false;
      case '$=':
        return mb_strpos($val1, $val2, 0, Xcrud_config::$mbencoding) ==
          mb_strlen($val1, Xcrud_config::$mbencoding) -
            mb_strlen($val2, Xcrud_config::$mbencoding)
          ? true
          : false;
      case "~=":
        return mb_strpos($val1, $val2, 0, Xcrud_config::$mbencoding) !== false
          ? true
          : false;
      default:
        return false;
  ...
  protected function create_modal($field, $content, $image = false)
  {
    ...
    if ($image) {
      $attr["data-content"] = $this->single_tag("img", "", [
        "alt" => "",
        "src" => $image,
      ]);
    } else {
  ...
  protected function _render_list_item($field, $value, $primary_val, $row)
  {
    ...
    if (isset($this->column_callback[$field])) {
      ...
      if (is_callable($this->column_callback[$field]["callback"]) && $row) {
        $value = call_user_func_array(
          $this->column_callback[$field]["callback"],
          [$value, $field, $primary_val, $row, $this]
        );
        return $value;
      }
    }
    ...
    if (isset($this->field_type[$field])) {
      switch ($this->field_type[$field]) {
        ...
        case "make_payment":
          $out .=
            "<button class='btn btn-primary' onclick='makePayment();'>Pay Bill to Selected Items</button>";
          break;
        case "bool":
        ...
        case "file":
          if ($value) {
            $out .= $this->open_tag("a", "", [
              "target" => "_blank",
              "href" => isset($this->upload_config[$field]["url"])
                ? $this->real_file_link($value, $this->upload_config[$field])
                : $this->file_link($field, $primary_val),
            ]);
        ...
        case "image":
          if ($value) {
            if (Xcrud_config::$images_in_grid) {
              $settings = $this->upload_config[$field];
              if (
                isset($settings["grid_thumb"]) &&
                isset($settings["thumbs"][$settings["grid_thumb"]])
              ) {
                $thumb = $settings["grid_thumb"];
              } else {
                $thumb = false;
              }
              $out .= $this->single_tag("img", "", [
                "alt" => "",
                "src" => isset($this->upload_config[$field]["url"])
                  ? $this->real_file_link($value, $this->upload_config[$field])
                  : $this->file_link(
                    $field,
                    $primary_val,
                ...
                "style" =>
                  "max-height: " . Xcrud_config::$images_in_grid_height . "px;",
              ]);
            } else {
              $out .= $this->open_tag("a", "", [
                "target" => "_blank",
              ...
              $out .= isset($this->upload_config[$field]["text"])
                ? $this->upload_config[$field]["text"]
                : $value;
              $out .= $this->close_tag("a");
            }
          }
          $image = isset($this->upload_config[$field]["url"])
            ? $this->real_file_link($value, $this->upload_config[$field])
            : $this->file_link($field, $primary_val, false, false, $value);
          break;
        case "remote_image":
          if ($value) {
            if (Xcrud_config::$images_in_grid) {
  ...
  protected function make_mailto($txt)
  {
    if ($this->emails_label) {
      return preg_replace(
        ...
        '<a target="_blank" href="mailto:$1@$2.$3">' .
          $this->emails_label["text"] .
          "</a>",
        $txt
      );
    } else {
  ...
  protected function make_links($txt)
  {
    if ($this->links_label !== false) {
      if ($this->links_label) {
        return preg_replace(
          "/(http:\/\/|https:\/\/)([^\s]+)/u",
          '<a target="_blank" href="$1$2">' .
            $this->links_label["text"] .
            "</a>",
          $txt
        );
      } else {
  ...
  protected function _render_export_item($field, $value, $primary_val, $row)
  {
    ...
    if (isset($this->column_callback[$field])) {
      ...
      if (is_callable($this->column_callback[$field]["callback"]) && $row) {
        $value = strip_tags(
          call_user_func_array($this->column_callback[$field]["callback"], [
            $value,
            $field,
            $primary_val,
            $row,
  ...
  {
    $out = "";
    if ($this->buttons_arrange == "dropdown-inline") {
      //default , dropdown-inline, dropdown-list){
      $rnd = rand();
      $styling = "";
      ...
      $out .=
        ...
        'type="button" data-bs-toggle="dropdown" id="dropdownMenu1' .
        $rnd .
        ...
        '" >' .
        "<li >";
    } elseif ($this->buttons_arrange == "dropdown-list") {
      $rnd = rand();
      $styling = "";
      if ($this->theme == "default") {
        $styling = "default-plain";
        ...
        if ($this->is_button($button["name"], $row)) {
          ...
          if ($button["params"]) {
            foreach ($button["params"] as $pkey => $pval) {
              $button["params"][$pkey] = $this->replace_text_variables(
                $pval,
                $row,
                true
              );
          ...
          if ($button["icon"]) {
            $out .=
              $this->open_tag("i", $button["icon"]) . $this->close_tag("i");
          } elseif ($this->theme_config("grid_default_icon")) {
            $out .=
              $this->open_tag("i", $this->theme_config("grid_default_icon")) .
              $this->close_tag("i");
          }

          if (
            Xcrud_config::$button_labels ||
            in_array($button["name"], $this->default_button_name)
          ) {
    ...
    if (
      !isset($this->hide_button["duplicate"]) &&
      !$this->table_ro &&
      $this->is_duplicate($row)
    ) {
      ...
      if ($this->theme_config("grid_duplicate_icon")) {
        $out .=
          $this->open_tag("i", $this->theme_config("grid_duplicate_icon")) .
          $this->close_tag("i");
      }
    ...
    if (!isset($this->hide_button["view"]) && $this->is_view($row)) {
      ...
      if ($this->theme_config("grid_view_icon")) {
        $out .=
          $this->open_tag("i", $this->theme_config("grid_view_icon")) .
          $this->close_tag("i");
      }
    ...
    if (
      ...
      if ($this->theme_config("grid_edit_icon")) {
        $out .=
          $this->open_tag("i", $this->theme_config("grid_edit_icon")) .
          $this->close_tag("i");
      }
    ...
    if (
      ...
      if ($this->theme_config("grid_remove_icon")) {
        $out .=
          $this->open_tag("i", $this->theme_config("grid_remove_icon")) .
          $this->close_tag("i");
      }
  ...
  protected function render_sum_item($field)
  {
    if (isset($this->sum_row[$field])) {
      if ($this->sum[$field]["custom"]) {
        return str_replace(
          "{value}",
          $this->_render_list_item($field, $this->sum_row[$field], 0, null),
          $this->sum[$field]["custom"]
        );
      } else {
        return $this->_render_list_item(
          $field,
          $this->sum_row[$field],
          0,
          null
  ...
  protected function _check_unique_value()
  {
    ...
    if ($out) {
      $data = ["error" => ["selector" => implode(",", $out)]];
    } else {
      $data = ["success" => 1];
    }
    //$data['key'] = $this->key;
  ...
  public static function check_url($url, $scr_url = false)
  {
    ...
    $scheme =
      (!isset($_SERVER["HTTPS"]) or
      !$_SERVER["HTTPS"] or
      strtolower($_SERVER["HTTPS"]) == "off" or
      strtolower($_SERVER["HTTPS"]) == "no")
        ? "http://"
        : "https://";
    ...
    $file_dir = trim(
      str_replace(
        "\\",
        "/",
        str_replace(["/public_html", "/private_html"], "", dirname(__FILE__))
      ),
      "/"
    );
    ...
    if ($is_full_url) {
      ...
      if (mb_substr($url, 0, 1) == "/" or mb_substr($url, 0, 2) == "./") {
        ...
        for ($i = 0; $i < $count; ++$i) {
          if (
            isset($script_uri_a[$i]) &&
            $script_uri_a[$i] == $request_uri_a[$i]
          ) {
  ...
  protected function file_link(
    ...
    $params = [
      "xcrud" => [
        ...
        "task" => "file",
        "rand" => base_convert(
          sha1($this->instance_name . ($filename ? $filename : rand())),
          10,
          36
        ),
      ],
    ...
    return Xcrud_config::$scripts_url .
      "/" .
  ...
  protected function real_file_link($filename, $params, $is_details = false)
  {
    ...
    if (
      $is_details &&
      isset($params["detail_thumb"]) &&
      isset($params["thumbs"][$params["detail_thumb"]])
    ) {
      ...
      !$is_details &&
      isset($params["grid_thumb"]) &&
      isset($params["thumbs"][$params["grid_thumb"]])
    ) {
  ...
  {
    if (
      is_array($this->table_info) &&
      count($this->table_info) &&
      !$this->table_ro
    ) {
      $db = Xcrud_db::get_instance($this->connection);
      $fields = [];
      ...
      if (!$this->primary_ai) {
        self::error(
          "Duplication impossible. Table does not have a primary autoincrement field."
        );
      }
      ...
      $table_join = $this->_build_table_join();
      $where_ai = $where
        ? "AND {$this->primary_ai} = " . (int) $this->primary_val
        : "WHERE {$this->primary_ai} = " . (int) $this->primary_val;
      $db->query(
        "SELECT {$select}\r\n FROM `{$this->table}`\r\n {$table_join}\r\n {$where}\r\n {$where_ai} LIMIT 1"
      );
      $postdata = $db->row();
  ...
  protected function _cell_attrib(
    $field,
    $value,
    $order,
    &$row,
      ...
      foreach ($this->fields_inline[$field] as $params) {
        ...
        if ($field == $compareVal) {
          ...
          $attr["identifier"] =
            ...
            $row["primary_key"] .
            "-" .
            $this->inline_edit_click .
            "-" .
            $this->inline_edit_save;
        }
      }
    }
    ...
    if ($column_class) {
      ...
      $attr["class"] = $this->replace_text_variables(
        $attr["class"],
        $row,
        true
      );
  ...
  protected function _get_table($method)
  {
    if (!$this->table && !$this->query) {
      self::error(
        "You must define your table before using the <strong>" .
          $method .
          "</strong> method."
      );
    } else {
  ...
  {
    // loads language array from ini file
    if (
      is_file(
        XCRUD_PATH .
          "/" .
  ...
  protected function _get_theme_config()
  {
    ...
    if (
      ...
      $this->theme_config = parse_ini_file(
        XCRUD_PATH .
          "/" .
          Xcrud_config::$themes_path .
          "/" .
  ...
  {
    $langtext = mb_convert_case(
      $text,
      MB_CASE_LOWER,
      Xcrud_config::$mbencoding
    );
    return htmlspecialchars(
      isset(self::$lang_arr[$langtext]) ? self::$lang_arr[$langtext] : $text,
      ENT_QUOTES,
      Xcrud_config::$mbencoding
    );
  }
  protected function theme_config($text = "")
  {
    ...
    return htmlspecialchars(
      isset($this->theme_config[$text]) ? $this->theme_config[$text] : "",
      ENT_QUOTES,
      Xcrud_config::$mbencoding
    );
  }
  ...
  public function _parse_field_names(
    $fields = "",
    $location = "",
    $table = false,
    $insert_prefix = true
    ...
    if ($fields) {
      ...
      if (is_array($fields)) {
        foreach ($fields as $key => $val) {
          if (is_int($key)) {
            if (!strpos($val, ".")) {
              $field_names[$this->make_field_alias($table, $val)] = [
                "table" => $table,
                "field" => $val,
              ];
            } else {
              $tmp = explode(".", $val, 2);
              $field_names[
                $this->make_field_alias($tmp[0], $tmp[1], $prefix)
              ] = ["table" => $prefix . $tmp[0], "field" => $tmp[1]];
              unset($tmp);
            }
          } else {
            if (!strpos($key, ".")) {
              $field_names[$this->make_field_alias($table, $key)] = [
                "table" => $table,
                "field" => $key,
                "value" => $val,
              ];
        ...
        foreach ($fields as $key => $val) {
          ...
          if (!strpos($val, ".")) {
            ...
            $tmp = explode(".", $val, 2);
            $field_names[$this->make_field_alias($tmp[0], $tmp[1], $prefix)] = [
              "table" => $prefix . $tmp[0],
              "field" => $tmp[1],
            ];
            unset($tmp);
      ...
      self::error(
        "You must set field name(s) for the <strong>" .
          $location .
          "</strong> method."
      );
    }
  ...
  public static function load_css()
  {
    ...
    if (self::$css_loaded) {
      self::error(
        'Xcrud\'s styles already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file'
      );
    }
    ...
    if (Xcrud_config::$load_bootstrap4) {
      ...
      $out .=
        ...
        Xcrud_config::$plugins_uri .
        '/Font-Awesome-fa-4/css/font-awesome.min.css" rel="stylesheet" type="text/css" />';
    } elseif (Xcrud_config::$load_semantic) {
      $out .=
        '<link href="' .
        Xcrud_config::$scripts_url .
        "/" .
    ...
    $out .=
      ...
      "/" .
      Xcrud_config::$plugins_uri .
      '/timepicker/jquery-ui-timepicker-addon.css" rel="stylesheet" type="text/css" /><link href="' .
      Xcrud_config::$scripts_url .
      "/" .
      Xcrud_config::$themes_uri .
      "/" .
      Xcrud_config::$theme .
      '/xcrud.css" rel="stylesheet" type="text/css" />';
    ...
    if (Xcrud_config::$load_filuploader) {
      ...
      $out .=
        ...
        "/" .
        Xcrud_config::$plugins_uri .
        '/jQuery-File-Upload-master/css/jquery.fileupload-ui.css" />';
      ...
      $out .=
        ...
        "/" .
        Xcrud_config::$plugins_uri .
        '/jQuery-File-Upload-master/css/jquery.fileupload-noscript.css"/></noscript>';
      $out .=
        '<noscript><link rel="stylesheet" href="' .
        Xcrud_config::$scripts_url .
        "/" .
        Xcrud_config::$plugins_uri .
        '/jQuery-File-Upload-master/css/jquery.fileupload-ui-noscript.css"/></noscript>';
    }
  ...
  public static function load_js()
  {
    ...
    if (self::$js_loaded) {
      self::error(
        'Xcrud\'s scripts already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file'
      );
    }
    self::$js_loaded = true;
    ...
    if (Xcrud_config::$load_filuploader) {
      ...
      $out .=
        ...
        "/" .
        Xcrud_config::$plugins_uri .
        '/jQuery-File-Upload-master/js/jquery.iframe-transport.js"></script>';
      ...
      $out .=
        ...
        "/" .
        Xcrud_config::$plugins_uri .
        '/jQuery-File-Upload-master/js/jquery.fileupload.js"></script>';
      ...
      $out .=
        ...
        "/" .
        Xcrud_config::$plugins_uri .
        '/jQuery-File-Upload-master/js/jquery.fileupload-process.js"></script>';
      ...
      $out .=
        ...
        "/" .
        Xcrud_config::$plugins_uri .
        '/jQuery-File-Upload-master/js/jquery.fileupload-image.js"></script>';
      ...
      $out .=
        ...
        "/" .
        Xcrud_config::$plugins_uri .
        '/jQuery-File-Upload-master/js/jquery.fileupload-audio.js"></script>';
      ...
      $out .=
        ...
        "/" .
        Xcrud_config::$plugins_uri .
        '/jQuery-File-Upload-master/js/jquery.fileupload-video.js"></script>';
      ...
      $out .=
        ...
        "/" .
        Xcrud_config::$plugins_uri .
        '/jQuery-File-Upload-master/js/jquery.fileupload-validate.js"></script>';
      ...
      $out .=
        ...
        "/" .
        Xcrud_config::$plugins_uri .
        '/jQuery-File-Upload-master/js/jquery.fileupload-ui.js"></script>';
      ...
      $out .=
        ...
        "/" .
        Xcrud_config::$plugins_uri .
        '/jQuery-File-Upload-master/js/demo.js"></script>';
    }
  ...
  protected function get_limit_list($limit = 20, $buttons = false)
  {
    ...
    if (!in_array($this->limit, $this->limit_list)) {
      $this->limit_list = array_merge([$this->limit], $this->limit_list);
    }
    if ($buttons) {
      $out .= $this->open_tag([
        "tag" => "div",
      ...
      foreach ($this->limit_list as $limts) {
        ...
        $out .=
          ...
          $this->lang($limts) .
          $this->close_tag(["tag" => "button"]);
      }
      $out .= $this->close_tag(["tag" => "div"]);
    } else {
      $out .= $this->open_tag(
        [
          "tag" => "select",
      ...
      foreach ($this->limit_list as $limts) {
        $tag = ["tag" => "option", "value" => $limts];
        if ($limts == $limit) {
          $tag["selected"] = "selected";
        }
        $out .=
          $this->open_tag($tag) . $this->lang($limts) . $this->close_tag($tag);
      }
      $out .= $this->close_tag(["tag" => "select"]);
    }
    return $out;
  }
  ...
  protected function check_file($path, $func_name)
  {
    ...
    if (!is_file($path)) {
      self::error(
        "Wrong path or file is not exist! The <strong>" .
          $func_name .
          "</strong> method fails.<br /><small>" .
          $path .
          "</small>"
      );
    }
  ...
  protected function check_folder($path, $func_name)
  {
    ...
    if (!is_dir($path)) {
      if (!@mkdir($path)) {
        self::error(
          "Wrong path or folder is not exist! The <strong>" .
            $func_name .
            "</strong> method fails.<br /><small>" .
            $path .
            "</small>"
  ...
  protected function additional_columns($fields = "")
  {
    if ($fields) {
      ...
      foreach ($fdata as $key => $fitem) {
        if (
          !isset($this->subselect[$key]) &&
          $fitem["field"] != "value" &&
          mb_substr_count($key, ".") < 2
        ) {
          if (!isset($this->columns[$key])) {
            $this->hidden_columns[$key] = $fitem;
          }
          $this->direct_select_tags[$key] = [
            "field" => $fitem["field"],
            "table" => $fitem["table"],
          ]; // will be get from db anyway
        }
      }
    }
  }
  ...
  protected function replace_text_variables(
    $value,
    array $data,
    $safety = false,
    $null_if_empty = false
  ...
  protected function send_http_request(
    $url,
    $data,
    $method,
    $return_result = false
    ...
    switch ($method) {
      case "get":
        $ch = curl_init(
          $path .
            (mb_strpos($path, "?", 0, Xcrud_config::$mbencoding) === false
              ? "?"
              : "&") .
            $data
        );
    ...
    if ($tag) {
      ...
      if (isset($tag["tag"])) {
        ...
        if ($enc_name && isset($tag["data-rangestart"])) {
          $tag["data-rangestart"] = $this->fieldname_encode(
            $tag["data-rangestart"]
          );
        }
        if ($enc_name && isset($tag["data-rangeend"])) {
          $tag["data-rangeend"] = $this->fieldname_encode(
            $tag["data-rangeend"]
          );
        }
        if ($tag) {
  ...
  protected function render_fields_list(
    $mode,
    $container = "table",
    $row = "tr",
    $label = "td",
    ...
    foreach ($unique_group as $key_grp => $item_grp) {
      ...
      if ($groupName != "") {
        $raw_out[] = $this->open_tag(
          $row,
          $this->theme_config("details_group_class")
        );
      }

      if ($group_contents[$key_grp] == true) {
        //show label
        //echo "FFFFF";
        if (!isset($this->field_tabs[$mode])) {
          if ($groupName != "") {
            $raw_out[] =
              $this->open_tag(
                $label,
                $this->theme_config("details_group_label"),
                ["colspan" => "2", "class" => "group_form_header"]
              ) .
              $groupName .
              $this->close_tag($label);
          }
      ...
      foreach (
        ...
        if (
          isset($this->fields[$item["name"]]["tab"]) &&
          $this->fields[$item["name"]]["tab"]
        ) {
          if (sizeof($this->fields_arrangement) > 0) {
            if (isset($this->fields_arrangement[$item["name"]]["group"])) {
              if (
                $this->fields_arrangement[$item["name"]]["group"] == $groupName
              ) {
                ...
                if (!$is_group_open) {
                  ...
                  if ($groupName == "{empty}") {
                    $tabs_out[$this->fields[$item["name"]]["tab"]][] =
                      $this->open_tag(
                        $label,
                        $this->theme_config("details_group_label"),
                        ["colspan" => "2", "class" => "group_form_header"]
                ...
                $tabs_out[$this->fields[$item["name"]]["tab"]][] =
                  $this->open_tag($row, $row_class) .
                  $this->open_tag(
                    $label,
                    $this->theme_config("details_label_cell")
                  ) .
                  $item["label"] .
                  $this->get_field_tooltip($item["name"], $mode) .
                  $this->close_tag($label) .
                  $this->open_tag(
                    $field,
                    $this->theme_config("details_field_cell")
                  ) .
                  $item["field"] .
                  $this->close_tag($field) .
                  $this->close_tag($row);
    ...
    if (isset($this->field_tabs[$mode]) or $this->default_tab !== false) {
      $tabs_header = "";
      $tabs_header =
        $this->open_tag($tabs_block, $this->theme_config("tabs_container"), [
          "class" => "xcrud-tabs",
        ]) .
        $this->open_tag($tabs_head, $this->theme_config("tabs_header_row"));
      $tabs_body = $this->open_tag(
        $tabs_content,
        $this->theme_config("tabs_content")
      );
      $k = 0;
      ...
      if ($this->default_tab !== false && $raw_out) {
        ...
        $tabs_header .=
          $this->open_tag(
            $tabs_row,
            $this->theme_config("tabs_header_cell") .
              ($k == 0 ? " " . $this->theme_config("tabs_first_element") : ""),
            ["data-tab" => "$tabid"]
          ) .
          $this->open_tag(
            $tabs_link,
            $this->theme_config("tabs_header_link") .
              ($k == 0 ? " " . $this->theme_config("tabs_first_element") : ""),
            ["href" => "#" . $tabid, "data-tab" => "$tabid"]
          ) .
          $this->default_tab .
          $this->close_tag($tabs_link) .
          $this->close_tag($tabs_row);
        $tabs_body .=
          $this->open_tag(
            $tabs_pane,
            $this->theme_config("tabs_content_pane") .
              ($k == 0 ? " " . $this->theme_config("tabs_first_element") : ""),
            ["id" => $tabid, "data-tab" => "$tabid"]
          ) .
          $this->open_tag(
            $container,
            $this->theme_config("details_container")
          ) .
          implode("", $raw_out) .
          $this->close_tag($container) .
          $this->close_tag($tabs_pane);
        ++$k;
        $raw_out = [];
      }
          ...
          if (isset($tabs_out[$tabname])) {
            ...
            $tabs_header .=
              $this->open_tag(
                ...
                $this->theme_config("tabs_header_cell") .
                  ($k == 0
                    ? " " . $this->theme_config("tabs_first_element")
                    : ""),
                ["data-tab" => "$tabid"]
              ) .
              $this->open_tag(
      ...
      $out .=
        $tabs_header .
        $this->close_tag($tabs_head) .
        $tabs_body .
        $this->close_tag($tabs_content) .
        $this->close_tag($tabs_block);
    }

    if ($raw_out) {
      $out .=
        $this->open_tag($container, $this->theme_config("details_container")) .
        implode("", $raw_out) .
        $this->close_tag($container);
    }
  ...
  /** autorender of details fields and tabs */
  protected function render_inline_edit_fields_list(
    $mode,
    $container = "table",
    $row = "tr",
    $label = "td",
  ...
  protected function get_table_tooltip()
  {
    ...
    if ($this->table_tooltip) {
      ...
      $out .= $this->open_tag([
        "tag" => "i",
        "class" => $this->table_tooltip["icon"]
          ? $this->table_tooltip["icon"]
  ...
  protected function get_field_tooltip($field, $mode)
  {
    ...
    if ($this->field_tooltip && isset($this->field_tooltip[$field])) {
      ...
      $out .= $this->open_tag([
        "tag" => "i",
        "class" => $this->field_tooltip[$field]["icon"]
          ? $this->field_tooltip[$field]["icon"]
          : $this->theme_config("tooltip_icon"),
      ]);
      $out .= $this->close_tag("i");
      $out .= $this->close_tag("a");
  ...
  protected function get_column_tooltip($field)
  {
    ...
    if ($this->column_tooltip && isset($this->column_tooltip[$field])) {
      ...
      $out .= $this->open_tag([
        "tag" => "i",
        "class" => $this->column_tooltip[$field]["icon"]
          ? $this->column_tooltip[$field]["icon"]
          : $this->theme_config("tooltip_icon"),
      ]);
      $out .= $this->close_tag("i");
      $out .= $this->close_tag("a");
    ...
    if ($this->is_search) {
      if (is_array($this->phrase) && isset($this->field_type[$this->column])) {
        switch ($this->field_type[$this->column]) {
          ...
          case "datetime":
            $phrase = [
              "from" => $this->unix2datetime((int) $this->phrase["from"], true),
              "to" => $this->unix2datetime((int) $this->phrase["to"], true),
            ];
            $is_daterange = true;
            break;
          case "date":
            $phrase = [
              "from" => $this->unix2date((int) $this->phrase["from"], true),
              "to" => $this->unix2date((int) $this->phrase["to"], true),
            ];
            $is_daterange = true;
            break;
          case "time":
            $phrase = [
              "from" => $this->unix2time((int) $this->phrase["from"], true),
              "to" => $this->unix2time((int) $this->phrase["to"], true),
            ];
            break;
          default:
            $phrase = "";
      ...
      $attr = ["class" => "xcrud-search-toggle", "href" => "javascript:;"];
      ...
      $attr = ["class" => "xcrud-search"];
      if (!$this->search && !Xcrud_config::$search_opened) {
        $attr["style"] = "display:none;";
      }
      $out .= $this->open_tag(
        "span",
        $this->theme_config("search_container"),
        $attr
      );
      if (Xcrud_config::$search_all) {
        $optlist[] =
          $this->open_tag("option", "", ["value" => ""]) .
          $this->lang("all_fields") .
          $this->close_tag("option");
      ...
      if ($this->search_columns) {
        foreach ($this->search_columns as $field => $tmp) {
          ...
          $attr = ["value" => $field, "data-type" => $this->field_type[$field]];
          if ($field == $this->column) {
            $attr["selected"] = "";
          }
          $optlist[] =
            $this->open_tag("option", "", $attr) .
            $name .
            $this->close_tag("option");
          $fieldlist = $this->search_fieldlist($field, $phrase, $fieldlist);
        }
      } elseif ($this->search_default !== false) {
        // not only 'all'
      ...
      $attr = ["class" => "xcrud-data", "name" => "column"];
      if ($this->search_default === false && !$this->search_columns) {
        $out .= $this->open_tag(
      ...
      if ($this->search) {
        ...
        $out .= $this->open_tag(
          "a",
          $this->theme_config("search_reset"),
          $attr
        );
  ...
  protected function render_search_tabulator()
  {
    ...
    if ($this->is_search) {
      ...
      $out .= $this->open_tag("a", $this->theme_config("search_open"), $attr);
      $out .= $this->lang("search") . $this->close_tag("a");
      $attr = ["class" => "xcrud-search ui form buttons"];
      if (!$this->search && !Xcrud_config::$search_opened) {
        $attr["style"] = "display:none;";
      }
      $out .= $this->open_tag(
  ...
  {
    $attr_preset = [
      "class" => $this->theme_config("xcrud_searchdata"),
      "name" => "phrase",
    ];
    if ($field == $this->column) {
    ...
    switch ($attr["data-type"]) {
      ...
      default:
        if (!isset($fieldlist["default"]) or $field == $this->column) {
          ...
          $attr["type"] = "text";
          $attr["value"] =
            (!is_array($phrase) && $field == $this->column or !$this->column)
              ? $phrase
              : "";
          $fieldlist["default"] = $this->single_tag(
            "input",
            $class . " " . $this->theme_config("search_phrase"),
            $attr
          );
        }
        break;
      case "bool":
        if (!isset($fieldlist["bool"]) or $field == $this->column) {
          ...
          $attr["type"] = "select";
          $fieldlist["bool"] = $this->open_tag(
            "select",
            $class . " " . $this->theme_config("search_phrase_dropdown"),
            $attr
          );
          $attr = ["value" => 1];
          if ($phrase == 1) {
      ...
      case "timestamp":
        if (!isset($fieldlist["date"]) or $field == $this->column) {
          ...
          $attr_range = [
            "class" =>
              "xcrud-daterange " .
              $this->theme_config("xcrud_searchdata") .
              "'" .
              $this->theme_config("search_range"),
            "name" => "range",
            "data-fieldtype" => "date",
          ];
          if ($field != $this->column) {
          ...
          $attr["value"] =
            isset($phrase["from"]) && $field == $this->column
              ? $phrase["from"]
              : "";
          $fieldlist["date"] .= $this->single_tag(
            "input",
            "xcrud-datepicker-from " .
              $class .
              " " .
              $this->theme_config("search_from"),
            $attr
          );
          $attr["name"] = "phrase][to";
          $attr["value"] =
            isset($phrase["to"]) && $field == $this->column
              ? $phrase["to"]
              : "";
          $fieldlist["date"] .= $this->single_tag(
            "input",
        ...
        if (is_array($this->field_attr[$field]["values"])) {
          foreach ($this->field_attr[$field]["values"] as $optkey => $opt) {
            if (is_array($opt)) {
              ...
              $opt_attr = ["value" => $optkey];
              if ($optkey == $phrase && $field == $this->column) {
                $opt_attr["selected"] = "";
              }
              $tmp .=
                $this->open_tag("option", "", $opt_attr) .
                $this->html_safe($opt) .
                $this->close_tag("option");
            }
          }
        } else {
          $opts = $this->parse_comma_separated(
            $this->field_attr[$field]["values"]
          );
          foreach ($opts as $opt) {
            $opt = trim($opt, '\'');
            $opt_attr = ["value" => $opt];
      ...
      foreach ($this->result_list as $key => $row) {
        ...
        if (
          ($this->is_edit ||
            ...
            $this->buttons ||
            $this->is_duplicate ||
            $this->grid_restrictions) &&
          $this->task != "print" &&
          $this->buttons_position == "left"
        ) {
        ...
        foreach ($this->columns as $field => $fitem) {
          ...
          if ($this->is_sortable) {
            ...
            if ($this->order_column == $field) {
              ...
              $attr["data-order"] =
                $this->order_direct == "asc" ? "desc" : "asc";
            } else {
          ...
          if (isset($this->tabulator_column_properties[$field])) {
            $extraAttr = explode(
              ",",
              $this->tabulator_column_properties[$field]
            );
          ...
          if ($this->is_numbers) {
            $tabulatorColObj["number"] = $this->_cell_attrib(
              false,
              false,
              false,
              $row,
        ...
        if (
          ($this->is_edit ||
            ...
            $this->grid_restrictions) &&
          $this->task != "print" &&
          $this->buttons_position == "right"
        ) {
          $out = "";
          $out .= $this->open_tag(
            $item,
            "xcrud-actions" .
              (Xcrud_config::$fixed_action_buttons ? " xcrud-fix" : ""),
            $this->_cell_attrib(
              false,
              false,
              false,
  ...
  protected function render_grid_head(
    $row = ["tag" => "tr"],
    $item = ["tag" => "th"],
    ...
    if ($this->is_numbers) {
      $out .=
        $this->open_tag($item, "xcrud-num") . "&#35;" . $this->close_tag($item);
    }
    if (
      ($this->is_edit ||
        $this->is_remove ||
      ...
      $out .=
        $this->open_tag($item, "xcrud-actions") .
        "&nbsp;" .
        $this->close_tag($item);
    }

    if ($this->is_bulk_select || $this->grid_restrictions) {
      $bs = isset($this->grid_restrictions["bulk_select"])
        ? $this->grid_restrictions["bulk_select"]
        : false;
  ...
  protected function render_grid_body(
    $row_tag = ["tag" => "tr"],
    $item = ["tag" => "td"]
  ) {
    $out = "";
    $i = 0;
    $unique_group_array_values = [];
    $unique_group_array_values_level2 = [];
    ...
    if (count($this->group_by_columns) > 0) {
      //has grouping columns
      if ($this->result_list) {
        foreach ($this->result_list as $key_group => $row_group) {
          foreach ($this->columns as $field => $fitem) {
            if (
              $field ==
              $this->group_by_columns[0]["table"] .
                "." .
                $this->group_by_columns[0]["field"]
            ) {
              $main_value = $row_group[$field];
              $main_field = $field;
              $rightVal = $this->_render_list_item(
                $main_field,
                $main_value,
                $row_group["primary_key"],
                $row_group
              );
        ...
        foreach ($unique_group_array_values as $key_group) {
          ...
          foreach ($this->result_list as $key => $row) {
            ...
            $listItemRelation_ = $this->_render_list_item(
              $main_field,
              $listItem_,
              $row["primary_key"],
              $row
            ...
            if ($key_group == $listItemRelation_) {
              ...
              $out_temp .= $this->open_tag(
                $row_tag,
                "xcrud-row xcrud-row-" . $i
              );
              if ($this->is_numbers) {
                $out_temp .=
                  $this->open_tag(
              ...
              if ($this->is_bulk_select || $this->grid_restrictions) {
                ...
                if (
                  $this->bulk_select_position == "left" &&
                  $this->is_bulk_select($row)
                ) {
              ...
              foreach ($this->columns as $field => $fitem) {
                ...
                if (count($this->group_sum_columns) > 0) {
                  if (
                    $this->group_sum_columns[0]["table"] .
                      "." .
                      $this->group_sum_columns[0]["field"] ==
                    $field
                  ) {
                ...
                if (!isset($this->column_hide[$field])) {
                  ...
                  $out_temp .= $this->_render_list_item(
                    $field,
                    $value,
                    $row["primary_key"],
                    $row
              ...
              //inline edit
              if (
                ($this->is_bulk_select || $this->grid_restrictions) &&
                $this->bulk_select_position == "right"
              ) {
                $out_temp .= $this->open_tag($item, "xcrud-actions");
                $tag = [
                  "tag" => "input",
          ...
          foreach ($this->columns as $field => $fitem) {
            ...
            if (
              ...
              $fieldname = $this->columns_names[$field];
              $out .=
                $this->open_tag($row_tag, "xcrud-row xcrud-grouping-row") .
                $this->open_tag($item, "", ["colspan" => $j]) .
                $fieldname .
                ": " .
                $key_group .
                " (" .
            ...
            foreach ($this->columns as $field => $fitem) {
              ...
              if (
                ...
                $out .=
                  $this->open_tag(
                    $row_tag,
                    "xcrud-row xcrud-grouping-row-sum"
                  ) .
                  $this->open_tag($item, "", ["colspan" => $field_pos]) .
                  "<span class='xcrud-total-title'>" .
                  $this->lang("total") .
                  "<span>" .
                  $this->close_tag($item) .
                  $this->open_tag($item, "", ["colspan" => 2]) .
                  "<span class='xcrud-total'>" .
                  $group_field_sum .
                  "</span>" .
                  $this->close_tag($item) .
                  $this->close_tag($row_tag);
        ...
        $out .=
          $this->open_tag($row_tag, "xcrud-row") .
          $this->open_tag($item, "", ["colspan" => $j]) .
          $this->lang("table_empty") .
          $this->close_tag($item) .
          $this->close_tag($row_tag);
      }
    } else {
        ...
        foreach ($this->result_list as $key => $row) {
          ...
          if ($this->is_bulk_select || $this->grid_restrictions) {
            $bs = isset($this->grid_restrictions["bulk_select"])
              ...
              : false;
            if (
              ($bs && $this->bulk_select_position == "left") ||
              ($this->is_bulk_select && $this->bulk_select_position == "left")
            ) {
              $out .= $this->open_tag($item, "xcrud-actions");
              if (
                $this->bulk_select_position == "left" &&
  ...
  protected function render_grid_footer(
    ...
    if ($this->sum && $this->result_list) {
      ...
      foreach ($this->columns as $field => $fitem) {
        ...
        $out .= $this->open_tag(
          $item,
          isset($this->sum[$field]) ? $this->sum[$field]["class"] : "",
          $this->_cell_attrib(
            $field,
  ...
  protected function render_benchmark($tag = ["tag" => "span"])
  {
    if ($this->benchmark) {
      return $this->open_tag($tag, "xcrud-benchmark") .
        $this->benchmark_end() .
        $this->close_tag($tag);
    }
    return "";
  ...
  protected function render_control_fields()
  {
    ...
    $out .= $this->single_tag($tag, "", [
      "name" => "limit",
      "value" => $this->limit ? $this->limit : Xcrud_config::$limit,
    ]);
    $out .= $this->single_tag($tag, "", [
      "name" => "instance",
  ...
  protected function render_callback()
  {
    ...
    if ($this->callback_url) {
      $out .=
        "<script id='callback_url'>window.location='" .
        $this->callback_url .
        "';</script>";
    }
    return $out;
  ...
  protected function render_button(
    ...
    if (!isset($this->hide_button[$name])) {
      ...
      if ($icon && !$this->is_rtl) {
        $out .=
          $this->open_tag(["tag" => "i", "class" => $icon]) .
          $this->close_tag("i") .
          " ";
      }
  ...
  protected function render_advanced_filter()
  {
    ...
    if ($this->is_search) {
      ...
      $attr = [
        "class" => $this->theme_config("xcrud_advanced_search_toggle"),
        "href" => "javascript:;",
      ];
      if ($this->search or Xcrud_config::$search_opened) {
        $attr["style"] = "display:none;";
      ...
      $attr = ["class" => "xcrud-advanced-search ui form"];
      if (!$this->search && !Xcrud_config::$search_opened) {
        $attr["style"] = "display:none;";
      }
      ...
      $out .= $this->open_tag(
        "div",
        $this->theme_config("advanced-search_container"),
        $attr
      );

      $attr = ["class" => "xcrud-advanced-search-title"];
      $out .= $this->open_tag(
        "span",
        $this->theme_config("advanced_search_title"),
        $attr
      );
      ...
      foreach ($this->advanced_filter as $field => $fitem) {
        ...
        if (is_array($this->advanced) || is_object($this->advanced)) {
          foreach ($this->advanced as $id => $v) {
            ...
            if ($field_spe == $original_field) {
              $fieldlist = $this->search_advanced_fieldlist(
                "",
                $id,
                $fieldlist
              );
        ...
        if ($this->field_type[$field] == "hidden") {
          ...
          if (isset($this->advanced_filter)) {
            $label =
              $this->html_safe($fitem["values"]) .
              (isset($this->validation_required[$fitem["values"]])
                ? "&#42;"
                : "");
          } else {
            if ($value != "") {
              ...
              if (isset($this->labels[$field])) {
                $label =
                  $this->html_safe($this->labels[$field]) .
                  (isset($this->validation_required[$field]) ? "&#42;" : "");
              } else {
                $label =
                  $this->html_safe($this->_humanize($field_n)) .
                  (isset($this->validation_required[$field_n]) ? "&#42;" : "");
              }
            }
          }
          ...
          $attr = ["class" => "xcrud-advanced-searchdata xcrud-input"];
          $this->fields_output[$field . "." . $count] = [
            "label" => $label,
            "field" => call_user_func_array(
              [$this, $func],
              [
      ...
      foreach ($this->fields_output as $field => $item) {
        ...
        $out .=
          ...
          $this->open_tag("label", $this->theme_config("details_label_cell")) .
          $item["label"] .
          $this->get_field_tooltip($item["name"], $mode) .
          $this->close_tag($label) .
          $this->open_tag($field, $this->theme_config("details_field_cell")) .
          $item["field"] .
          $this->close_tag($field) .
          "</div>";
      }
      ...
      $out .= $this->open_tag(
        "a",
        $this->theme_config("advanced_search_go"),
        $attr
      );
      $out .= $this->lang("go") . $this->close_tag("a");
      ...
      $attr = [
        "class" =>
          "xcrud-advanced-search-toggle col-auto btn btn-light btn-warning ui white button",
        "href" => "javascript:;",
        "data-search" => 0,
      ];
      $out .= $this->open_tag(
        "a",
        $this->theme_config("advanced_search_reset"),
        $attr
      );
  ...
  /** renders action button for details view */
  protected function render_next(
    $name = "",
    $task = "",
    $after = "",
    $class = "",
  ...
  /** renders action button for details view */
  protected function render_previous(
    $name = "",
    $task = "",
    $after = "",
    $class = "",
  ...
  protected function refresh_button($class = "", $icon = "")
  {
    if (
      $this->is_refresh &&
      !isset($this->hide_button["refresh"]) &&
      !$this->table_ro
    ) {
  ...
  protected function get_image_folder($field)
  {
    ...
    if (isset($settings["path"])) {
      ...
      $path = $this->check_folder(
        Xcrud_config::$upload_folder_def,
        "get_image_folder"
      );
    }
  ...
  protected function safe_file_name($file, $field)
  {
    ...
    if (
      isset($this->upload_config[$field]["not_rename"]) &&
      $this->upload_config[$field]["not_rename"] == true
    ) {
      $filename = $this->_clean_file_name($file["name"]);
    } else {
      $filename =
        base_convert(str_replace(" ", "", microtime()) . rand(), 10, 36) . $ext;
    }
    return $filename;
  }
  ...
  protected function get_filename_noconfict($filename, $field)
  {
    ...
    if (is_file($file_path)) {
      $filename = substr_replace(
        $filename,
        "_" . base_convert(time() . rand(), 10, 36),
        strrpos($filename, "."),
        0
      );
    }
  ...
  {
    if (
      isset($this->upload_config[$field]["width"]) or
      isset($this->upload_config[$field]["height"]) or
      /* or isset($this->upload_config[$field]['field'])*/ isset(
        $this->upload_config[$field]["manual_crop"]
      ) &&
        $this->upload_config[$field]["manual_crop"] ==
          true /* or (isset($this->upload_config[$field]['thumbs']) && count($this->
  ...
  {
    if (
      isset($this->upload_config[$field]["manual_crop"]) &&
      $this->upload_config[$field]["manual_crop"] == true
    ) {
      return true;
    } else {
      return false;
  ...
  protected function remove_tmp_image($filename, $field)
  {
    ...
    if (
      isset($this->upload_config[$field]["save_original"]) &&
      $this->upload_config[$field]["save_original"] == true
    ) {
      if (
        isset($this->upload_config[$field]["original_marker"]) &&
        !empty($this->upload_config[$field]["original_marker"])
      ) {
        $orig_filename = $this->_thumb_name(
          $filename,
          $this->upload_config[$field]["original_marker"]
        );
      } else {
        $orig_filename = $this->_thumb_name($filename, "_orig");
      }
      rename(
        $this->get_image_folder($field) . "/" . $tmp_filename,
        $this->get_image_folder($field) . "/" . $orig_filename
      );
    } else {
      $path = $this->get_image_folder($field);
  ...
  protected function unix2date($time, $utc = false)
  {
    if ($time) {
      return $utc
        ? gmdate($this->date_format["php_d"], $time)
        : date($this->date_format["php_d"], $time);
    } else {
      return "";
    }
  }
  protected function unix2datetime($time, $utc = false)
  {
    if ($time) {
      return $utc
        ? gmdate(
          $this->date_format["php_d"] . " " . $this->date_format["php_t"],
          $time
        )
        : date(
          Xcrud_config::$php_date_format . " " . $this->date_format["php_t"],
          $time
        );
    } else {
      return "";
  ...
  protected function unix2time($time, $utc = false)
  {
    if ($time) {
      return $utc
        ? gmdate($this->date_format["php_t"], $time)
        : date($this->date_format["php_t"], $time);
    } else {
      return "";
    }
  }
  ...
  protected function render_table_name(
    $mode = "list",
    $tag = "h2",
    $to_show = false
  ) {
    ...
    if ($this->is_title) {
      ...
      switch ($mode) {
        case "create":
          $out .= $this->is_rtl
            ? "<small>" . $this->lang("add") . " - </small>" . $this->table_name
            : $this->table_name .
              "<small> - " .
              $this->lang("add") .
              "</small>";
      ...
      if (Xcrud_config::$can_minimize) {
        if ($to_show) {
          $out .=
            '<span class="xcrud-toggle-show xcrud-toggle-down"><i class="' .
            $this->theme_config("slide_down_icon") .
            '"></i></span>';
        } else {
          $out .=
            '<span class="xcrud-toggle-show xcrud-toggle-up"><i class="' .
            $this->theme_config("slide_up_icon") .
            '"></i></span>';
        }
      }
      $out .= $this->close_tag($tag);
  ...
  public function encrypt($obj)
  {
    if (!Xcrud_config::$alt_encription_key) {
      self::error(
        'Please, set <strong>$alt_encription_key</strong> parameter in configuration file'
      );
    }
    ...
    if (defined("MCRYPT_TWOFISH") && mcrypt_module_self_test(MCRYPT_TWOFISH)) {
      ...
      mcrypt_module_self_test(MCRYPT_RIJNDAEL_256)
    ) {
      $algoritm = MCRYPT_RIJNDAEL_256;
    } elseif (
      defined("MCRYPT_SERPENT") &&
      mcrypt_module_self_test(MCRYPT_SERPENT)
    ) {
      $algoritm = MCRYPT_SERPENT;
    } elseif (
      defined("MCRYPT_BLOWFISH") &&
      mcrypt_module_self_test(MCRYPT_BLOWFISH)
    ) {
  ...
  protected function is_edit(&$row)
  {
    if (!isset($this->grid_restrictions["edit"])) {
      ...
      $fdata = $this->_parse_field_names(
        $this->grid_restrictions["edit"]["field"]
      );
      $fname = key($fdata) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
      return $this->_compare(
        $row[$fname],
        $this->grid_restrictions["edit"]["operator"],
        $this->replace_text_variables(
          $this->grid_restrictions["edit"]["value"],
          $row
        )
      )
        ? $this->is_edit
  ...
  protected function is_bulk_select(&$row)
  {
    if (!isset($this->grid_restrictions["bulk_select"])) {
      ...
      $fdata = $this->_parse_field_names(
        $this->grid_restrictions["bulk_select"]["field"]
      );
      $fname = key($fdata) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
      return $this->_compare(
        $row[$fname],
        $this->grid_restrictions["bulk_select"]["operator"],
        $this->replace_text_variables(
          $this->grid_restrictions["bulk_select"]["value"],
          $row
        )
      )
        ? $this->is_bulk_select
  ...
  protected function is_remove(&$row)
  {
    if (!isset($this->grid_restrictions["remove"])) {
      ...
      $fdata = $this->_parse_field_names(
        $this->grid_restrictions["remove"]["field"]
      );
      $fname = key($fdata) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
      return $this->_compare(
        $row[$fname],
        $this->grid_restrictions["remove"]["operator"],
        $this->replace_text_variables(
          $this->grid_restrictions["remove"]["value"],
          $row
        )
      )
        ? $this->is_remove
  ...
  protected function is_duplicate(&$row)
  {
    if (!isset($this->grid_restrictions["duplicate"])) {
      ...
      $fdata = $this->_parse_field_names(
        $this->grid_restrictions["duplicate"]["field"]
      );
      $fname = key($fdata) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
      return $this->_compare(
        $row[$fname],
        $this->grid_restrictions["duplicate"]["operator"],
        $this->replace_text_variables(
          $this->grid_restrictions["duplicate"]["value"],
          $row
        )
      )
        ? $this->is_duplicate
  ...
  protected function is_view(&$row)
  {
    if (!isset($this->grid_restrictions["view"])) {
      ...
      $fdata = $this->_parse_field_names(
        $this->grid_restrictions["view"]["field"]
      );
      $fname = key($fdata) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
      return $this->_compare(
        $row[$fname],
        $this->grid_restrictions["view"]["operator"],
        $this->replace_text_variables(
          $this->grid_restrictions["view"]["value"],
          $row
        )
      )
        ? $this->is_view
  ...
  protected function is_button($name, &$row)
  {
    if (isset($this->grid_restrictions[$name])) {
      $fdata = $this->_parse_field_names(
        $this->grid_restrictions[$name]["field"]
      );
      $fname = key($fdata) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
      return $this->_compare(
        $row[$fname],
        $this->grid_restrictions[$name]["operator"],
        $this->replace_text_variables(
          $this->grid_restrictions[$name]["value"],
          $row
        )
      )
        ? true
  ...
  protected function _call_action()
  {
    ...
    if (isset($this->action[$name])) {
      ...
      if (is_callable($this->action[$name]["callable"])) {
        call_user_func_array($this->action[$name]["callable"], [$this]);
      }
    }
    $this->task = $this->after;
    $this->after = null;
  ...
  public function fieldname_decode($name = "")
  {
    return str_replace(
      "`",
      "",
      base64_decode(str_replace(["-", "_", ":"], ["=", "/", "+"], $name))
    );
  }
  ...
  protected function cast_number_format($number, $field, $edit = false)
  {
    ...
    $number = preg_replace(
      "/^(.*)[\.\," .
        preg_quote($this->field_attr[$field]["point"], "/") .
        "]+([^\.\," .
        preg_quote($this->field_attr[$field]["point"], "/") .
        ']*)$/ui',
      '$1' . $loc_point . '$2',
